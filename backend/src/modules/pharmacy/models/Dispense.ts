...
const LineSchema = new Schema({
  ...
  packMode: { type: String, enum: ['loose', 'pack'], default: 'loose' },
...
const DispenseSchema = new Schema({
  ...
  lineGstTotal: { type: Number, default: 0 },
  billGstType: { type: String, enum: ['percent', 'fixed'], default: 'percent' },
  billGstValue: { type: Number, default: 0 },
  billGstAmount: { type: Number, default: 0 },
...
export type DispenseDoc = {
  ...
  profit?: number
  createdBy?: string
  lines: { medicineId: string; name: string; unitPrice: number; qty: number; costPerUnit?: number; discountRs?: number; packMode?: 'loose'|'pack'; lineTaxType?: 'percent'|'fixed'; lineTaxValue?: number }[]
  fbrInvoiceNo?: string
  fbrQrCode?: string
  fbrStatus?: string
  fbrMode?: string
...
export const Dispense = (models.Pharmacy_Dispense || model('Pharmacy_Dispense', DispenseSchema)) as any
