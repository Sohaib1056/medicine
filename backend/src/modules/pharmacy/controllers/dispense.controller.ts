...
export async function create(req: Request, res: Response) {
  ...
  const subtotal = data.subtotal ?? data.lines.reduce((s: number, l: { unitPrice: number; qty: number }) => s + l.unitPrice * l.qty, 0)
  const lineDiscount = Number(data.lineDiscountTotal || (data as any).lineDiscountTotal || 0)
  const lineGstTotal = Number(data.lineGstTotal || 0)
  const billDiscount = ((Math.max(0, subtotal - lineDiscount)) * (data.discountPct || 0)) / 100
  const billGstAmount = Number(data.billGstAmount || 0)
  const total = data.total ?? (subtotal - lineDiscount - billDiscount + lineGstTotal + billGstAmount)
  ...
  for (const line of data.lines) {
    ...
    linesWithCost.push({
      ...line,
      costPerUnit,
      discountRs: Number((line as any)?.discountRs || 0),
      packMode: (line as any)?.packMode || 'loose',
      lineTaxType: (line as any)?.lineTaxType || 'percent',
      lineTaxValue: Number((line as any)?.lineTaxValue || 0)
    })
  ...
  // Auto-create/update customer profile if phone is provided
  let customerIdFinal = data.customerId
  const phoneInput = String(data.customerPhone || '').trim()
  const nameInput = String(data.customer || 'Walk-in').trim()
  
  if (phoneInput && phoneInput !== 'Walk-in') {
    try {
      let existing = await Customer.findOne({ phone: { $regex: phoneInput, $options: 'i' } })
      if (existing) {
        customerIdFinal = String(existing._id)
        if (nameInput && nameInput !== 'Walk-in' && (!existing.name || existing.name === 'Walk-in')) {
          existing.name = nameInput
          await existing.save()
        }
      } else {
        const created = await Customer.create({ name: nameInput, phone: phoneInput })
        customerIdFinal = String(created._id)
      }
    } catch (e) {
      console.error('Error auto-creating customer profile:', e)
    }
  }
  ...
  const doc: any = await Dispense.create({
    ...
    billGstType: data.billGstType || 'percent',
    billGstValue: data.billGstValue || 0,
    lineDiscountTotal: lineDiscount,
    lineGstTotal: lineGstTotal,
    billGstAmount: billGstAmount,
  ...
  for (const line of data.lines) {
    try {
      ...
      const dec = isPack ? Number(line.qty || 0) * unitsPerPack : Number(line.qty || 0)

      item.onHand = Math.max(0, Number(item.onHand || 0) - dec)

      if (line.unitPrice != null) {
        if (isPack) {
          item.lastSalePerPack = Number(line.unitPrice)
          item.lastSalePerUnit = Number(line.unitPrice) / unitsPerPack
        } else {
          item.lastSalePerUnit = Number(line.unitPrice)
          if (unitsPerPack > 0) {
            item.lastSalePerPack = Number(line.unitPrice) * unitsPerPack
          }
        }
      }
      await item.save()
