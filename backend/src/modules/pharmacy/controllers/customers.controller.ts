...
export async function getHistory(req: Request, res: Response) {
  ...
  const sales = await Dispense.find({ customerId: id }).sort({ createdAt: -1 }).lean()
  res.json(sales)
}

export async function getByPhone(req: Request, res: Response) {
  const phone = String(req.params.phone || '').trim()
  if (!phone) return res.status(400).json({ error: 'Phone number required' })
  const c = await Customer.findOne({ phone: { $regex: phone, $options: 'i' } }).lean()
  if (!c) return res.status(404).json({ error: 'Customer not found' })
  const lastDispense = await Dispense.findOne({ customerId: String((c as any)._id) }).sort({ createdAt: -1 }).lean()
  res.json({ customer: c, lastDispense })
}
