...
export async function list(req: Request, res: Response) {
  ...
  const { from, to, search, company, limit } = parsed.success ? parsed.data as any : {}
  const filter: any = {}
  if (from || to) {
    filter.date = {}
  ...
  if (company) {
    const rx = new RegExp(company, 'i')
    const or = filter.$or || []
    or.push({ companyName: rx })
    or.push({ companyId: rx })
    filter.$or = or
  }
        ...
        data: [
          ...
          { $limit: limit },
          {
            $project: {
              ...
              'manufacturer': '$lines.manufacturer',
...
export async function update(req: Request, res: Response) {
  ...
  const data = draftCreateSchema.parse(req.body)
  const { lines, totals } = calcTotals(data.lines, (data as any).discount || 0, data.invoiceTaxes || [])
  const prevMapByName = new Map<string, any>()
  const prevLines: any[] = ((exists as any)?.lines || []) as any[]
  for (const l of (prevLines || [])) {
    const k = String(l.name || '').trim().toLowerCase()
    if (!k) continue
    prevMapByName.set(k, l)
  }
  const clientLines: any[] = ((data as any).lines || []) as any[]
  const mergedLines: any[] = (lines as any[]).map((l: any) => {
    const k = String(l.name || '').trim().toLowerCase()
    const prev = prevMapByName.get(k)
    const orderedUnitsFromClient = (clientLines || []).find((dl: any) =>
      String(dl?.name || '').trim().toLowerCase() === k)?.orderedUnits
    const orderedUnits = (orderedUnitsFromClient != null)
      ? Number(orderedUnitsFromClient)
      : Number(prev?.orderedUnits || 0)
    return { ...l, orderedUnits }
  })
...
export async function approve(req: Request, res: Response) {
  ...
  for (const l of (draft.lines || [])) {
    ...
    if (l.manufacturer) update.$set.manufacturer = l.manufacturer
    ...
    if (l.image) {
      update.$set.image = l.image
      console.log('✅ BACKEND APPROVE: Saving image to inventory:', name, '| Image path:', l.image)
    } else {
      console.log('❌ BACKEND APPROVE: No image in draft line for:', name)
    }
    ...
    console.log('💾 BACKEND APPROVE: About to save inventory item:', name, '| Has image:', !!l.image)
  ...
  const purchase = await Purchase.create({
    ...
    sourceType: draft.sourceType || (String(draft.invoice || '').startsWith('PO-') ? 'PO' : undefined),
    sourceRef: draft.sourceRef || (String(draft.invoice || '').startsWith('PO-') ? String(draft.invoice || '').slice(3) : undefined),
...
export async function approveLine(req: Request, res: Response) {
  const { id, lineId } = req.params
  const draft: any = await PurchaseDraft.findById(id).lean()
  if (!draft) return res.status(404).json({ error: 'Draft not found' })

  const lineIndex = (draft.lines || []).findIndex((l: any) => String(l._id) === lineId)
  if (lineIndex === -1) return res.status(404).json({ error: 'Line not found' })

  const l = draft.lines[lineIndex]
  ...
  if (name) {
    ...
    if (l.defaultDiscountPct != null) update.$set.defaultDiscountPct = Math.max(0, Math.min(100, Number(l.defaultDiscountPct)))
    if (salePerUnit) update.$set.lastSalePerUnit = salePerUnit
    if (l.image) update.$set.image = l.image

    if (expiry) {
      update.$min = { earliestExpiry: expiry }
    }
    await InventoryItem.findOneAndUpdate({ key }, update, { upsert: true, new: true })
  }

  // 2) Create Purchase record for this ONE line
  const lineNet = (l.buyPerPack || 0) * (l.packs || 0) // Basic approximation if not using full tax calc for single line
  // Ideally we should use calcTotals for just this one line to get net/afterTax correctly
  const { lines: pLines, totals: pTotals } = calcTotals([l], 0, [])
  ...
  try {
    ...
    await AuditLog.create({
      actor,
      action: 'Add Inventory',
      label: 'ADD_INVENTORY_LINE',
      method: 'POST',
      path: req.originalUrl,
      at: new Date().toISOString(),
      detail: `Approved Line: ${l.name} from Invoice ${purchase.invoice} — Rs ${Number(purchase.totalAmount || 0).toFixed(2)}`,
    })
  } catch { }

  // 3) Remove line from draft or delete draft
  const remainingLines = draft.lines.filter((_: any, idx: number) => idx !== lineIndex)
  if (remainingLines.length === 0) {
    await PurchaseDraft.deleteOne({ _id: id })
  } else {
    // Recalculate totals for remaining lines
    const { lines: updatedLines, totals: updatedTotals } = calcTotals(remainingLines, (draft as any).discount || 0, draft.invoiceTaxes || [])
    await PurchaseDraft.findByIdAndUpdate(id, {
      $set: {
        lines: updatedLines,
...
export async function rejectLine(req: Request, res: Response) {
  const { id, lineId } = req.params
  const draft: any = await PurchaseDraft.findById(id).lean()
  if (!draft) return res.status(404).json({ error: 'Draft not found' })

  const remainingLines = (draft.lines || []).filter((l: any) => String(l._id) !== lineId)
