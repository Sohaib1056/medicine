...
export async function create(req: Request, res: Response){
  ...
  if (data.type === 'Customer'){
    ...
    for (const r of data.lines){
      ...
      // STRICT CHECK: Item must exist in inventory to be returned
      let item: any = null
      ...
      if (!item) throw new ApiError(400, `Item ${name} not found in current inventory. Returns are only allowed for items existing in inventory.`)
    ...
    // Restock inventory for returned items (we already found the items above, but we'll re-find for simplicity in this loop or we could have stored them)
    for (const r of data.lines){
      try {
        const id = String(r.medicineId || '').trim()
        const name = String(r.name || '').trim()
  ...
  if (data.type === 'Supplier'){
    ...
    for (const r of data.lines){
      ...
      // STRICT CHECK: Item must exist in inventory to be returned to supplier
      let item: any = null
      if (id && mongoose.isValidObjectId(id)) item = await InventoryItem.findById(id)
      if (!item && name) item = await InventoryItem.findOne({ key: name.toLowerCase() })
      if (!item && id) item = await InventoryItem.findOne({ lastMedicineId: id })
      if (!item) throw new ApiError(400, `Item ${name} not found in current inventory. Returns are only allowed for items existing in inventory.`)
      if (item.onHand < qtyToReturn) throw new ApiError(400, `Not enough stock in inventory for ${name}. Available: ${item.onHand}, Return: ${qtyToReturn}`)
