...
export async function listPayments(req: Request, res: Response){
  try{
    ...
    const rows = await HospitalErPayment.find({ encounterId: enc._id }).sort({ receivedAt: -1 }).skip((page-1)*limit).limit(limit)
...
export async function createPayment(req: Request, res: Response){
  try{
    ...
    const row = await HospitalErPayment.create({ ...data, encounterId: enc._id, patientId: (enc as any).patientId })
