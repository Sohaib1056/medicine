...
export async function getById(req: Request, res: Response){
  ...
  const row = await HospitalPrescription.findById(String(id))
    .populate({ path: 'encounterId', select: 'doctorId patientId startAt', populate: [{ path: 'doctorId', select: 'name' }, { path: 'patientId', select: 'fullName mrn phoneNormalized' }] })
    .lean()
...
export async function getByEncounter(req: Request, res: Response){
  ...
  const enc = await HospitalEncounter.findById(String(encounterId)).lean()
  if (!enc) return res.status(404).json({ error: 'Encounter not found' })
  const row = await HospitalPrescription.findOne({ encounterId: String(encounterId) })
    .populate({ path: 'encounterId', select: 'doctorId patientId startAt', populate: [{ path: 'doctorId', select: 'name' }, { path: 'patientId', select: 'fullName mrn' }] })
    .lean()
  if (!row) return res.status(404).json({ error: 'Prescription not found' })
  res.json({ prescription: row })
...
export async function byEncounterQuery(req: Request, res: Response){
  const encounterId = (req.query?.encounterId || '').toString()
  if (!encounterId) return res.status(400).json({ error: 'encounterId required' })
  const row = await HospitalPrescription.findOne({ encounterId })
    .populate({ path: 'encounterId', select: 'doctorId patientId startAt', populate: [{ path: 'doctorId', select: 'name' }, { path: 'patientId', select: 'fullName mrn' }] })
    .lean()
  if (!row) return res.status(404).json({ error: 'Prescription not found' })
  res.json({ prescription: row })
...
export async function upsertForEncounter(req: Request, res: Response){
  const { encounterId } = req.params as any
  const enc = await HospitalEncounter.findById(String(encounterId))
  if (!enc) return res.status(404).json({ error: 'Encounter not found' })
  if (enc.type !== 'OPD') return res.status(400).json({ error: 'Only OPD encounters can have prescriptions' })
  const data = req.body || {}
  const att: any = (data as any).manualAttachment
  ...
  const set: any = {
    ...
    manualAttachment: att,
    items: (data as any).items || [],
    labTests: (data as any).labTests,
    labNotes: (data as any).labNotes,
    diagnosticTests: (data as any).diagnosticTests,
    diagnosticNotes: (data as any).diagnosticNotes,
    primaryComplaint: (data as any).primaryComplaint,
    primaryComplaintHistory: (data as any).primaryComplaintHistory,
    ...
    examFindings: (data as any).examFindings,
    diagnosis: (data as any).diagnosis,
    advice: (data as any).advice,
    vitals: (data as any).vitals,
    // optional extras ignored by schema but acceptable
    medicationNotes: (data as any).medicationNotes,
    improvement: (data as any).improvement,
    followupDate: (data as any).followupDate,
    detailsExtras: (data as any).detailsExtras,
    customEntries: (data as any).customEntries,
    aorticSelected: (data as any).aorticSelected,
  }
  const existing = await HospitalPrescription.findOne({ encounterId: String(encounterId) })
  let row
  if (existing) {
    row = await HospitalPrescription.findByIdAndUpdate(existing._id, { $set: set }, { new: true })
  } else {
    row = await HospitalPrescription.create({
      patientId: enc.patientId,
  ...
  const full = await HospitalPrescription.findById(row._id)
    .populate({ path: 'encounterId', select: 'doctorId patientId startAt', populate: [{ path: 'doctorId', select: 'name' }, { path: 'patientId', select: 'fullName mrn' }] })
    .lean()
