...
const ResultRowSchema = new Schema({
  ...
  value: { type: String },
  flag: { type: String, enum: ['normal','low','high','critical_low','critical_high','abnormal','critical'], required: false },
  comment: { type: String },
}, { _id: false })
...
export type LabResultDoc = {
  _id: string
  orderId: string
  rows: Array<{ test: string; normal?: string; unit?: string; prevValue?: string; value?: string; flag?: 'normal'|'low'|'high'|'critical_low'|'critical_high'|'abnormal'|'critical'; comment?: string }>
