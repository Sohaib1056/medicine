...
export async function list(req: Request, res: Response){
  ...
  const skip = (pg - 1) * lim
  // Join with Lab_Patient to ensure MRN is always present in the response
  const pipeline: any[] = [
    { $match: filter },
    { $sort: { createdAt: -1 } },
    { $skip: skip },
    ...
    },
    {
      $addFields: {
        patient: {
          $mergeObjects: [
            '$patient',
            { mrn: { $ifNull: ['$patient.mrn', { $arrayElemAt: ['$pat.mrn', 0] }] } }
          ]
        }
      }
    },
  ...
  const [items, total] = await Promise.all([
    (LabOrder as any).aggregate(pipeline),
...
export async function create(req: Request, res: Response){
  ...
  try {
    const snap: any = (data as any).patient || {}
    if (!snap.mrn) {
      const pid = String((data as any).patientId || '')
      if (pid) {
        const { LabPatient } = await import('../models/Patient')
        const p: any = await (LabPatient as any).findById(pid).select('mrn').lean()
        if (p?.mrn) (data as any).patient = { ...snap, mrn: String(p.mrn) }
      }
    }
  } catch {}
...
export async function collectPayment(req: Request, res: Response){
  ...
  const base: any = await LabOrder.findById(id).lean()
  if (!base) return res.status(404).json({ message: 'Order not found' })

  const tokenNo = String(base.tokenNo || '')
  if (!tokenNo) return res.status(400).json({ message: 'Token not found for this order' })

  const orders: any[] = await LabOrder.find({ tokenNo }).sort({ createdAt: 1 }).lean()
  if (!orders.length) return res.status(404).json({ message: 'No orders found for token' })

  const totalDue = orders.reduce((s, o) => s + Math.max(0, Number(o.receivableAmount || 0)), 0)
  if (totalDue <= 0) {
    return res.json({ tokenNo, appliedAmount: 0, totalDueBefore: 0, totalDueAfter: 0, orders })
  }

  const applyAmt = Math.min(totalDue, Math.max(0, Number(data.amount || 0)))
  let remaining = applyAmt

  // Apply payment to orders with receivableAmount > 0, in createdAt order
  for (const o of orders) {
    const due = Math.max(0, Number(o.receivableAmount || 0))
    if (due <= 0) continue
    if (remaining <= 0) break
    const pay = Math.min(due, remaining)
    remaining -= pay

    const nextReceived = Math.max(0, Number(o.receivedAmount || 0) + pay)
    const nextReceivable = Math.max(0, Number(o.receivableAmount || 0) - pay)

    await LabOrder.findByIdAndUpdate(String(o._id), {
      $set: {
  ...
  try {
    ...
    await LabAuditLog.create({
      ...
      label: 'COLLECT_PAYMENT',
      method: 'POST',
      path: req.originalUrl,
      at: new Date().toISOString(),
      detail: `Token ${tokenNo} — collected=${applyAmt}${data.note ? ` — ${data.note}` : ''}`,
    })
  } catch {}

  const updated: any[] = await LabOrder.find({ tokenNo }).sort({ createdAt: 1 }).lean()
  const totalDueAfter = updated.reduce((s, o) => s + Math.max(0, Number(o.receivableAmount || 0)), 0)
  res.json({ tokenNo, appliedAmount: applyAmt, totalDueBefore: totalDue, totalDueAfter, orders: updated })
}
