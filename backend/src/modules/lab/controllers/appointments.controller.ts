...
export async function create(req: Request, res: Response) {
  try {
    ...
    const apptDateIso = String((data as any).dateIso || (data as any).appointmentDate || '').slice(0, 10)
    if (!apptDateIso) return res.status(400).json({ error: 'Appointment date is required' })
...
export async function update(req: Request, res: Response) {
  try {
    ...
    const patch: any = {}
    const incomingDate = (data as any).appointmentDate != null ? (data as any).appointmentDate : (data as any).dateIso
    if (incomingDate != null) patch.dateIso = String(incomingDate).slice(0, 10)
    if (data.time != null) patch.time = data.time
    if ((data as any).priority != null) patch.priority = (data as any).priority
