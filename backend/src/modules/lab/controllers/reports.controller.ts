...
export async function summary(req: Request, res: Response){
  ...
  const [ordersInRange, expensesInRange, purchasesAll] = await Promise.all([
    LabOrder.find({ createdAt: { $gte: from, $lte: to } }).select('createdAt tests net status paymentMethod receivedAmount').lean(),
  ...
  const methodKeys = ['Cash','Card','Bank','Online'] as const
  const paymentsByMethod = methodKeys.reduce((acc:any, k)=> (acc[k] = 0, acc), {} as Record<string, number>)
  ordersInRange.forEach((o:any)=>{
    const m = methodKeys.includes(String(o.paymentMethod) as any) ? String(o.paymentMethod) : 'Cash'
    paymentsByMethod[m] = (paymentsByMethod[m] || 0) + Number(o.receivedAmount ?? o.net ?? 0)
  })
  for (const k of methodKeys) paymentsByMethod[k] = Number((paymentsByMethod[k] || 0).toFixed(2))
  ...
  const dailyMethodMap = new Map<string, Record<string, number>>()
  ordersInRange.forEach((o:any)=>{
    const d = new Date(o.createdAt as any)
    const key = fmt(d)
    const m = methodKeys.includes(String(o.paymentMethod) as any) ? String(o.paymentMethod) : 'Cash'
    const cur = dailyMethodMap.get(key) || { Cash: 0, Card: 0, Bank: 0, Online: 0 }
    cur[m] = (cur[m] || 0) + Number(o.receivedAmount ?? o.net ?? 0)
    dailyMethodMap.set(key, cur)
  })
  for (let d = new Date(from); d <= to; d = new Date(d.getTime()+24*60*60*1000)){
    const key = fmt(d)
    if (!dailyMethodMap.has(key)) dailyMethodMap.set(key, { Cash: 0, Card: 0, Bank: 0, Online: 0 })
  }
  const dailyRevenueByPaymentMethod = Array.from(dailyMethodMap.entries())
    .sort((a,b)=> a[0]<b[0]? -1 : 1)
    .map(([date, by])=>({
      date,
      Cash: Number((by.Cash||0).toFixed(2)),
      Card: Number((by.Card||0).toFixed(2)),
      Bank: Number((by.Bank||0).toFixed(2)),
      Online: Number((by.Online||0).toFixed(2)),
    }))
