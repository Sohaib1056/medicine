...
import { LayoutDashboard, CalendarCheck, Users, Settings as Cog, Sparkles, History, FileText, Boxes, Truck, Receipt, UserCog, ScrollText, Bell, ChevronRight, LogOut } from 'lucide-react'
...
export default function Aesthetic_Sidebar({ collapsed = false }: { collapsed?: boolean }) {
  ...
  return (
    ...
    >
      <nav className={`scrollbar-hide-on-hover flex-1 overflow-y-auto overflow-x-hidden ${collapsed ? 'p-2' : 'p-3'} space-y-1`}>
        {(() => {
          const out: any[] = []
        ...
        <div className={collapsed ? 'mt-2 mb-2' : 'mt-4 mb-2'}>
          <SwitchPortal collapsed={collapsed} currentPortal="aesthetic" />
        </div>
        <button
          onClick={logout}
