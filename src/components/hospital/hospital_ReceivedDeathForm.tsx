...
export default function Hospital_ReceivedDeathForm({ encounterId, patient, isER }: ReceivedDeathFormProps){
  ...
  useEffect(()=>{ (async()=>{
    ...
    try {
      setLoading(true)
      const res: any = isER
        ? await hospitalApi.getErReceivedDeath(encounterId)
        : await hospitalApi.getIpdReceivedDeath(encounterId)
      ...
      // Also fetch patient CNIC from encounter if not already present (skip for ER)
      if (!isER && !form.patientCnic) {
        try {
  ...
  const save = async () => {
    ...
    if (isER) {
      await hospitalApi.upsertErReceivedDeath(encounterId, payload)
    } else {
  ...
  const printPreview = async () => {
    if (isER) {
      ...
      w.document.write(`<!DOCTYPE html><html><head><title>Received Death Form</title>
        ...
        <div class="header"><h1>RECEIVED DEATH FORM</h1><p>BIN BARKAT HOSPITAL</p></div>
        <div class="section"><span class="label">Patient:</span> <span class="value">${patient?.name || '-'}</span> | <span class="label">MRN:</span> <span class="value">${patient?.mrn || '-'}</span></div>
        <div class="section"><span class="label">Diagnosis:</span> <span class="value">${form.diagnosis || '-'}</span></div>
        <div class="section"><span class="label">Death Declared By:</span> <span class="value">${form.deathDeclaredBy || '-'}</span></div>
        <div class="section"><span class="label">Attendant Name:</span> <span class="value">${form.attendantName || '-'}</span> (${form.attendantRelation || '-'})</div>
        <div class="section"><span class="label">Charge Nurse:</span> <span class="value">${form.chargeNurseName || '-'}</span></div>
        <div class="section"><span class="label">Doctor:</span> <span class="value">${form.doctorName || '-'}</span></div>
