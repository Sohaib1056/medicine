...
export default function Medication({ encounterId }: { encounterId: string }){
  ...
  const [isIpdAdmission, setIsIpdAdmission] = useState(false)

  useEffect(()=>{ 
    if(!encounterId){ 
      setIsIpdAdmission(false)
      return 
    }
    // Check if this is actually an IPD admission before loading medications
    hospitalApi.getIPDAdmissionByIdSilent(encounterId).then((res: any) => {
      if (res?.encounter?._id) {
        setIsIpdAdmission(true)
        reload()
      } else {
        setIsIpdAdmission(false)
      }
