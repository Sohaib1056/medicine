...
type Props = { encounterId?: string; patient?: any; isER?: boolean; onSaved?: () => void };
...
export default function Discharged(props: Props){
  ...
  useEffect(() => { (async () => {
    ...
    // If props provided, use them; else resolve via :id (skip IPD lookup for ER)
    try {
  ...
  const save = async (doPrint?: boolean) => {
    if (!encounterId && !isER) return;
    const tokenId = String(props.encounterId || id || '');
    if (!tokenId) return;
    ...
    if (isER) {
      await hospitalApi.upsertErDischargeSummary(tokenId, payload as any)
      if (doPrint) await previewHtml(apiBase + '/hospital/er/tokens/' + encodeURIComponent(tokenId) + '/discharge-summary/print')
    } else {
        ...
        <div className="mt-4 flex flex-wrap gap-2 justify-end">
          <button onClick={() => {
            if (isER) {
              ...
              const medsList = meds.map(m => [m.name, m.dose, m.route, m.freq, m.timing, m.duration].filter(Boolean).join(' | ')).filter(Boolean)
              const course = [
                presentingComplaints && `Presenting Complaints: ${presentingComplaints}`,
                reasonOfAdmission && `Reason of Admission: ${reasonOfAdmission}`,
                treatmentInHospital && `Treatment: ${treatmentInHospital}`,
              ...
              w.document.write(`<!DOCTYPE html><html><head><title>Discharge Summary</title>
                ...
                <div class="header"><h1>DISCHARGE SUMMARY</h1><p>BIN BARKAT HOSPITAL</p></div>
                <div class="section"><span class="label">Patient:</span> <span class="value">${patient?.name || '-'}</span> | 
                  <span class="label">MRN:</span> <span class="value">${patient?.mrn || '-'}</span> | 
                  <span class="label">Phone:</span> <span class="value">${patient?.phone || '-'}</span></div>
                <div class="section"><span class="label">Date of Release:</span> <span class="value">${dor || '-'}</span></div>
                <div class="section"><span class="label">Course in Hospital:</span><br/><pre style="white-space:pre-wrap;font-family:inherit">${course || '-'}</pre></div>
                <div class="section"><span class="label">Final Diagnosis:</span> <span class="value">${finalDiagnosis || '-'}</span></div>
                <div class="section"><span class="label">Condition at Discharge:</span> <span class="value">${condAtDischarge || '-'}</span></div>
                <div class="section"><span class="label">Medicines:</span><br/>${medsList.length ? medsList.join('<br/>') : '-'}</div>
                <div class="section"><span class="label">Notes:</span><br/><pre style="white-space:pre-wrap;font-family:inherit">${notesBlob || '-'}</pre></div>
                <div class="section"><span class="label">Doctor:</span> <span class="value">${doctorName || '-'}</span> | 
                  <span class="label">Date:</span> <span class="value">${signDate || '-'}</span></div>
                <script>window.print()</script>
                </body></html>`)
              w.document.close(); w.focus()
              return
            }
            if (!encounterId) return; previewHtml(apiBase + '/hospital/ipd/admissions/' + encodeURIComponent(encounterId) + '/discharge-summary/print');
          }} className="btn-outline-navy">Print</button>
