...
const NOTE_PREFIX = '[SURGICAL_SAFETY_SIGNOUT]:'
...
export default function Hospital_IpdSurgicalSafetySignOut({ encounterId }: { encounterId: string }) {
  ...
  async function reload() {
    ...
    setError(null)
    try {
      const res = (await hospitalApi.listIpdNotes(encounterId, { page: 1, limit: 200 })) as any
      const rowsRaw = (res?.items || res?.data || res || []) as any
      const rows = Array.isArray(rowsRaw) ? rowsRaw : []

      const filtered: RecordItem[] = []
      for (const n of rows) {
        const text = String(n?.text || '')
        if (!text.startsWith(NOTE_PREFIX)) continue
        const payload = text.slice(NOTE_PREFIX.length)
        try {
          filtered.push({ id: String(n?._id || n?.id || Math.random()), createdAt: n?.createdAt, parsed: JSON.parse(payload) })
        } catch {
          filtered.push({ id: String(n?._id || n?.id || Math.random()), createdAt: n?.createdAt, parsed: null })
        }
      }
      setItems(filtered)
    } catch (e: any) {
  ...
  async function save(form: any) {
    ...
    setError(null)
    try {
      await hospitalApi.createIpdNote(encounterId, { noteType: 'nursing', text: `${NOTE_PREFIX}${JSON.stringify(form)}` })
      setOpen(false)
      await reload()
    } catch (e: any) {
