...
export default function Hospital_DeathCertificateForm({ encounterId, patient, isER }: DeathCertificateFormProps){
  ...
  const save = async () => {
    ...
    if (isER) {
      await hospitalApi.upsertErDeathCertificate(encounterId, payload)
    } else {
  ...
  const saveAndPrint = async () => {
    if (isER) {
      ...
      w.document.write(`<!DOCTYPE html><html><head><title>Death Certificate</title>
        ...
        <div class="header"><h1>DEATH CERTIFICATE</h1><p>BIN BARKAT HOSPITAL</p></div>
        <div class="section"><span class="label">DC No:</span> <span class="value">${form.dcNo || '-'}</span></div>
        <div class="section"><span class="label">Patient:</span> <span class="value">${patient?.name || '-'}</span> | <span class="label">MRN:</span> <span class="value">${form.mrNumber || patient?.mrn || '-'}</span></div>
        <div class="section"><span class="label">Date of Death:</span> <span class="value">${form.dateOfDeath || '-'}</span> | <span class="label">Time:</span> <span class="value">${form.timeOfDeath || '-'}</span></div>
        <div class="section"><span class="label">Cause of Death - Primary:</span><br/>${form.primaryCause || '-'}</div>
        <div class="section"><span class="label">Cause of Death - Secondary:</span><br/>${form.secondaryCause || '-'}</div>
        <div class="section"><span class="label">Body Received By:</span> <span class="value">${form.receiverName || '-'}</span> (${form.receiverRelation || '-'})</div>
