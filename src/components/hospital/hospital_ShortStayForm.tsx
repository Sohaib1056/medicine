...
export default function Hospital_ShortStayForm(props: Props){
  ...
  useEffect(()=>{ (async()=>{
    try {
      ...
      if (encId){
        ...
        // Skip IPD API calls for ER encounters
        if (!isER) {
      ...
      // Fallback: resolve from route id (skip for ER)
      if (!routeId || isER) return
  ...
  })() }, [props.encounterId, props.patient, routeId, isER])
  ...
  async function save(){
    ...
    const tokenId = String(props.encounterId || '');
    if (!tokenId) return;
    setSaving(true)
    try {
      if (isER) {
        await hospitalApi.upsertErShortStay(tokenId, {
