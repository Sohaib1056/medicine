...
export default function InvoicePage(_props: { patientId?: string; embedded?: boolean; isER?: boolean }){
  ...
  // Load ER final invoice if isER (skip for IPD)
  useEffect(()=>{ if(!id || !isER) return; (async()=>{
    try {
      setLoading(true);
      const res: any = await hospitalApi.getErFinalInvoice(id).catch(()=>null as any);
      const invData = res?.invoice;
      if (invData) {
        setCurrentId(invData._id);
        setInv({
          refNo: invData.refNo || '',
          mrn: invData.mrn || '',
          patientName: invData.patientName || '',
          employeeName: invData.employeeName || '',
          relationWithPatient: invData.relationWithPatient || '',
          bps: invData.bps || '',
          designation: invData.designation || '',
          employeeNo: invData.employeeNo || '',
          procedure: invData.procedure || '',
          dateOfAdmission: invData.dateOfAdmission || '',
          dateOfDischarge: invData.dateOfDischarge || '',
          daysOccupied: invData.daysOccupied || 0,
          lineItems: invData.lineItems || [],
          totalAmount: invData.totalAmount || 0,
          discount: invData.discount || 0,
          totalPayable: invData.totalPayable || 0,
          currency: invData.currency || 'PKR',
        });
        if (invData.dischargeTime) setDischargeTime(invData.dischargeTime);
      }
    } finally {
      setLoading(false);
    }
  ...
  // Load IPD encounter patient + billing items (skip for ER)
  useEffect(()=>{ if(!id || isER) return; (async()=>{
    try {
      setLoading(true);
  ...
  async function save(){
    ...
    try {
      if (isER) {
        // Save ER Final Invoice to database
        const payload = {
          ...inv,
          dischargeTime,
        };
        const res: any = await hospitalApi.upsertErFinalInvoice(id, payload);
        if (res?.invoice?._id) {
          setCurrentId(res.invoice._id);
        }
        return;
      }
