...
export default function Pharmacy_AddSupplierDialog({ open, onClose, onSave, initial = null, title = 'Add Supplier', submitLabel = 'Save' }: Props) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // Focus name input after dialog opens and data (companies) is potentially loaded
        if (mounted) {
          setTimeout(() => {
            nameInputRef.current?.focus()
          }, 50)
        ...
        if (!mounted) return
        setCompanies([])
        // Still try to focus even if company load fails
        if (mounted) {
          setTimeout(() => {
            nameInputRef.current?.focus()
          }, 50)
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      // Find the next focusable element (excluding hidden/disabled)
      for (let i = index + 1; i < form.elements.length; i++) {
        const el = form.elements[i] as HTMLElement
        if (el.tagName !== 'BUTTON' && (el instanceof HTMLInputElement || el instanceof HTMLSelectElement || el instanceof HTMLTextAreaElement) && !el.hasAttribute('disabled')) {
          e.preventDefault()
          el.focus()
          break
        }
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/5">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          <div>
            ...
            <input ref={nameInputRef} name="name" defaultValue={initial?.name} required onKeyDown={handleKeyDown} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-slate-700">Company</label>
            <select value={selectedCompanyId} onChange={e=>setSelectedCompanyId(e.target.value)} onKeyDown={handleKeyDown} className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900">
          ...
          <div>
            ...
            <input name="phone" defaultValue={initial?.phone} onKeyDown={handleKeyDown} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-slate-700">Address</label>
            <textarea name="address" defaultValue={initial?.address} rows={3} onKeyDown={handleKeyDown} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-slate-700">Tax ID</label>
            <input name="taxId" defaultValue={initial?.taxId} onKeyDown={handleKeyDown} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-slate-700">Status</label>
            <select name="status" defaultValue={initial?.status ?? 'Active'} onKeyDown={handleKeyDown} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
