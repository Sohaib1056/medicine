  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        let arr1: any[] = []
        let arr2: any[] = []
        try { const r1 = await pharmacyApi.listSupplierPurchases(supplier.id); arr1 = r1.items || [] } catch (e) { console.error('Error fetching by ID:', e) }
        if (supplier.name){ try { const r2 = await pharmacyApi.listPurchases({ search: supplier.name, limit: 500 }); arr2 = r2.items || [] } catch (e) { console.error('Error fetching by name:', e) } }
        if (!mounted) return
                ...
                <tbody className="divide-y divide-slate-200">
                  ...
                  {!loading && purchases.map((p:any)=> (
                    <tr key={p._id}>
                      ...
                      <td className="px-3 py-2">Rs {Number(p.totalAmount||0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="px-3 py-2">Rs {Number(p.paid||0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="px-3 py-2">Rs {Number(p.remaining||0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-slate-200">
                  ...
                  {!loading && rows.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td className="px-3 py-2">{r.buyPerPack.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="px-3 py-2">{r.unitBuy.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="px-3 py-2">{r.salePerPack.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="px-3 py-2">{r.unitSale.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="px-3 py-2">{(r.profitPerUnit||0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
