...
export default function Doctor_Header({ onToggle, onToggleTheme, theme, variant = 'default' }: Props) {
  ...
  const headerCls = isNavy
    ? 'h-14 w-full'
    : 'h-14 border-b border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900/80'
  ...
  const metaTextCls = isNavy ? 'ml-auto flex items-center gap-3 text-sm text-white/85' : 'flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300'
  ...
  const badgeCls = isNavy ? 'rounded-full bg-white/10 px-2 py-1 text-xs font-medium text-white' : 'rounded-full bg-slate-800 px-2 py-1 text-xs font-medium text-white'
  ...
  return (
    <header className={headerCls}>
      <div className={innerCls}>
        ...
        <div className={isNavy ? 'text-sm font-semibold text-white' : 'text-sm font-semibold text-slate-800 dark:text-slate-200'}>Doctor Portal</div>
        ...
        <div className={metaTextCls}>
          ...
          <div className={badgeCls}>{doc?.username?.slice(0,2).toUpperCase() || 'DR'}</div>
        </div>
      </div>
    </header>
  )
