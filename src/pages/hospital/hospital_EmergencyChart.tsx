...
function SearchSelect({ options, value, onChange, placeholder }: { options: Option[]; value: string; onChange: (v: string) => void; placeholder?: string }) {
...
export default function Hospital_EmergencyChart(){
  ...
  const goReferral = () => {
    setToast({ type: 'info', message: 'Refer to IPD will be wired after backend confirmation' })
  }
  ...
  const discharge = () => {
    navigate(`/hospital/discharge/${tokenId || ''}?type=er`)
  }
                ...
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {loadingCharges ? (
                      ...
                      return (
                      <tr key={c.id}>
                        <td className="px-3 py-2 text-xs text-slate-500">{dateStr}</td>
                        <td className="px-3 py-2 text-xs text-slate-500">{timeStr}</td>
          ...
          >
            ...
            <div className="space-y-3 px-5 py-4 text-sm">
              <div>
                <label className="block text-xs font-medium text-slate-600">Service</label>
                <SearchSelect
                  options={svcCatalog.map(s => ({ value: s.name, label: `${s.name} (Rs${s.price})` }))}
                  value={selectedService}
                  onChange={(v) => {
                    setSelectedService(v)
                    const svc = svcCatalog.find(s => s.name === v)
                    if (svc) setSelectedPrice(String(svc.price))
                  }}
                  placeholder="Select service..."
                />
                <input type="hidden" name="description" value={selectedService} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-600">Qty</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600">Rate</label>
                  <input name="unitPrice" type="number" value={selectedPrice} onChange={e => setSelectedPrice(e.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
                </div>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 border-t border-slate-200 px-5 py-3">
              <button type="button" onClick={()=>{setOpenCharge(false); setSelectedService(''); setSelectedPrice('0')}} className="btn-outline-navy">Cancel</button>
