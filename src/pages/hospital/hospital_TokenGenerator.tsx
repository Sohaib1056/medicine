...
export default function Hospital_TokenGenerator() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (form.billingType === 'Corporate' && form.corporateCompanyId) {
        ...
        try {
          ...
          // Use schedule fee if a schedule is selected, otherwise fall back to doctor/department base fee
          const selectedSchedule = scheduleId ? schedules.find(s => String(s._id) === String(scheduleId)) : null
          let base = Number(selectedSchedule?.fee) > 0 ? Number(selectedSchedule?.fee) : NaN
          if (!Number.isFinite(base) || base <= 0) {
            ...
            base = Number.isFinite(docBase!) && Number(docBase) > 0 ? Number(docBase) : (Number.isFinite(depBase!) && Number(depBase) > 0 ? Number(depBase) : NaN)
          }
      ...
      try {
        // First check if we have a selected schedule with a fee
        const selectedSchedule = scheduleId ? schedules.find(s => String(s._id) === String(scheduleId)) : null
        const scheduleFee = selectedSchedule?.fee
        if (Number.isFinite(Number(scheduleFee)) && Number(scheduleFee) > 0) {
          if (!cancelled) setForm(prev => ({ ...prev, consultationFee: String(scheduleFee) }))
          return
        }
        // Fall back to API quote if no schedule fee
    ...
    return () => { cancelled = true }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.departmentId, form.doctor, form.corporateCompanyId, form.billingType, scheduleId, schedules])
    ...
    <div>
      ...
      <form onSubmit={generateToken} className="mt-6 space-y-8">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button type="submit" className="rounded-md bg-violet-700 px-4 py-2 text-sm font-medium text-white hover:bg-violet-800">{isEditMode ? 'Update Token' : 'Generate Token'}</button>
        </div>
      </form>
      {showSlip && slipData && (
