...
import Hospital_VitalsModal from '../../components/hospital/Hospital_VitalsModal'
...
export default function Hospital_TodayTokens(){
  ...
  const [vitalsModal, setVitalsModal] = useState<{ open: boolean; token: TokenRow | null }>({ open: false, token: null })
  ...
  return (
    <div>
              ...
              <tr key={r._id} className={`text-slate-700 ${r.status === 'returned' ? 'bg-amber-50' : ''}`}>
                ...
                <Td>
                  <div className="flex gap-2">
                    <button onClick={() => setVitalsModal({ open: true, token: r })} title="Vitals" className="rounded-md bg-emerald-600 px-2 py-1 text-[10px] font-bold text-white hover:bg-emerald-700 uppercase">Vitals</button>
      ...
      {vitalsModal.open && vitalsModal.token && (
        <Hospital_VitalsModal
          open={vitalsModal.open}
          onClose={() => setVitalsModal({ open: false, token: null })}
          tokenId={vitalsModal.token._id}
          patientName={vitalsModal.token.patient}
          onSaved={() => setToast({ type: 'success', message: 'Vitals saved successfully' })}
        />
      )}
    </div>
  )
