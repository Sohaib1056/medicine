...
export default function Doctor_Layout() {
  ...
  const [collapsed, setCollapsed] = useState<boolean>(()=>{
    try { return localStorage.getItem('doctor.sidebar.collapsed') === '1' } catch { return false }
  })
  ...
  const toggle = () => {
    setCollapsed(v=>{
      const nv = !v
      try { localStorage.setItem('doctor.sidebar.collapsed', nv ? '1' : '0') } catch {}
      return nv
    })
  }
      ...
      <div className={shell}>
        <div className="sticky top-0 z-20 w-full md:border-b" style={{ background: 'linear-gradient(180deg, var(--navy) 0%, var(--navy-700) 100%)', borderColor: 'rgba(255,255,255,0.12)' }}>
          <div className="flex h-14">
            <Doctor_Header variant="navy" onToggle={toggle} onToggleTheme={() => setTheme(t=>t==='dark'?'light':'dark')} theme={theme} />
          </div>
        </div>
