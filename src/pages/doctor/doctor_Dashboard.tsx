...
import { hospitalApi, getApiBase } from '../../utils/api'
...
export default function Doctor_Dashboard() {
  ...
  async function load(){
    if (!doc?.id || !isObjectId(doc.id)) { setQueuedCount(0); setPrescCount(0); setLabRefCount(0); setPhRefCount(0); setDiagRefCount(0); setQueue([]); setRecentPres([]); return }
    try {
