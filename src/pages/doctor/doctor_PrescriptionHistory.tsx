...
import { downloadPrescriptionPdf, getSavedPrescriptionPdfTemplate } from '../../utils/prescriptionPdf'
...
export default function Doctor_PrescriptionHistory() {
  ...
  const [printNoHeader, setPrintNoHeader] = useState(false)
  ...
  async function handlePrint(id: string) {
    setPrintNoHeader(false)
    const data: any = await fetchPrintData(id)
    setPrintData(data)
    setPreviewOpen(true)
  }

  async function handleNoHeaderPrint(id: string) {
    try {
      setPrintNoHeader(true)
      const data: any = await fetchPrintData(id)
      setPrintData(data)
      setPreviewOpen(true)
    } catch {
      setToast({ msg: 'Failed to load prescription for print', kind: 'error' })
    }
  }
  ...
  useEffect(() => {
    if (!pendingBrowserPrint) return
    if (!printData) return
    const printViaIframe = () => {
      const el = document.getElementById('prescription-print-history')
      if (!el) return false
      ...
      doc.open()
      doc.write(`<!doctype html><html><head><base href="${baseHref}">${headHtml}</head><body>${bodyHtml}</body></html>`)
      doc.close()
      ...
      // Give the iframe time to load CSS/fonts/images and paint before printing.
      const triggerPrint = () => {
        try {
          win.focus()
          win.print()
      ...
      const waitForReadyAndPrint = async () => {
        try {
          // Wait for DOM ready
          const start = Date.now()
          while (doc.readyState !== 'complete' && Date.now() - start < 2000) {
          ...
          try {
            ...
            await Promise.race([
              ...
              new Promise<void>(res => setTimeout(res, 750)),
            ])
          } catch { }
        } catch { }
      ...
      try {
        // Start waiting after a short delay to allow CSS to attach
        setTimeout(() => { void waitForReadyAndPrint() }, 150)
      } catch {
        triggerPrint()
      }
  ...
  return (
    <div className="space-y-4">
            ...
            {mine.map(p => (
              <div key={p.id} className="px-4 py-3 text-sm">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  <button className="rounded-md border border-slate-300 px-3 py-1 text-sm" onClick={() => handleNoHeaderPrint(p.id)}>No Header Print</button>
                  ...
                  <button
                    ...
                    onClick={() => navigate(`/doctor/prescription?prescriptionId=${encodeURIComponent(p.id)}`)}
                  >Edit</button>
      ...
      {/* Print Preview Modal */}
      {previewOpen && printData && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
          <div className="w-full max-w-5xl rounded-xl bg-slate-100 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4 rounded-t-xl sticky top-0 z-10">
              <div className="text-lg font-bold text-slate-800">Print Preview</div>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => { setPendingBrowserPrint(true); setPreviewOpen(false); }}
                  className="rounded-md bg-sky-600 px-6 py-2 text-sm font-bold text-white shadow-sm hover:bg-sky-700"
                >
                  Print Now
                </button>
                <button
            ...
            <div className="p-6 overflow-x-auto">
              <div className="mx-auto bg-white shadow-sm border border-slate-200">
                  <PrescriptionPrint
                    isPreview={true}
                    noHeaderPrint={printNoHeader}
                    doctor={printData.doctor || {}}
                    settings={printData.settings || { name: '', address: '', phone: '' }}
                    patient={printData.patient || {}}
                    items={(printData.items || []).map((m: any) => ({
                      ...m,
                      route: m.route || (m.notes?.toLowerCase().includes('route:') ? m.notes.split(';').find((s: string) => s.toLowerCase().includes('route:'))?.split(':')[1]?.trim() : undefined)
                    }))}
...
async function fetchPrintData(id: string) {
  let [detail, settings] = await Promise.all([
  ...
  return { settings, doctor, patient, items: pres?.items || [], vitals: pres?.vitals, labTests: pres?.labTests || [], labNotes: pres?.labNotes, diagnosticTests: pres?.diagnosticTests || [], diagnosticNotes: pres?.diagnosticNotes, primaryComplaint: pres?.primaryComplaint || pres?.complaints, primaryComplaintHistory: pres?.primaryComplaintHistory, familyHistory: pres?.familyHistory, allergyHistory: pres?.allergyHistory, treatmentHistory: pres?.treatmentHistory, history: pres?.history, examFindings: pres?.examFindings, diagnosis: pres?.diagnosis, advice: pres?.advice, medicationNotes: (pres as any)?.medicationNotes, improvement: (pres as any)?.improvement, followupDate: (pres as any)?.followupDate, createdAt: pres?.createdAt }
}
