...
export default function Pharmacy_PurchaseHistory() {
  ...
  const [returnsIndex, setReturnsIndex] = useState<Record<string, number>>({})
  const [returnNamesByInvoice, setReturnNamesByInvoice] = useState<Record<string, string[]>>({})

  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-purchase-history-search') as HTMLInputElement | null)?.focus() }, 0)
    ...
    const load = async () => {
      try {
        ...
        const mapped: Row[] = items.map((p: any) => {
          ...
          const origTotal = l.totalItems || ((l.unitsPerPack || 1) * (l.packs || 0)) || 0
          const retKey = String(p.invoice || '').trim() + '|' + String(l.name || '').trim().toLowerCase()
          const retQty = returnsIndex[retKey] || 0
          const adjTotal = Math.max(0, Number(origTotal) - Number(retQty || 0))
          const nameFallback = (() => {
            const inv = String(p.invoice || '').trim()
            const arr = returnNamesByInvoice[inv] || []
            return arr.length ? arr.join(', ') : '-'
          })()
          return {
            id: p._id,
            date: p.date,
            medicine: l.name || nameFallback,
        ...
        // Calculate total purchase amount from filtered results
        const totalAmount = mapped.reduce((sum: number, row: Row) => sum + (row.totalAmount || 0), 0)
        setTotalPurchaseAmount(totalAmount)
      } catch {}
    }
    load()
  }, [searchTick, from, to, search, page, limit, returnsIndex])

  useEffect(() => {
    let mounted = true
    ;(async () => {
      try {
        const first: any = await pharmacyApi.listReturns({ type: 'Supplier', from: from || undefined, to: to || undefined, page: 1, limit: 1000 })
        let items: any[] = first?.items ?? first ?? []
        const tp = Number(first?.totalPages || 1)
        if (tp > 1) {
          const pages = Array.from({ length: tp - 1 }, (_, i) => i + 2)
          const results = await Promise.all(pages.map(p => pharmacyApi.listReturns({ type: 'Supplier', from: from || undefined, to: to || undefined, page: p, limit: 1000 })))
          for (const r of results) items = items.concat((r?.items ?? r ?? []))
        }
        const idx: Record<string, number> = {}
        const namesByInv: Record<string, Set<string>> = {}
        for (const ret of items) {
          const ref = String(ret.reference || '').trim()
          const lines: any[] = ret.lines || []
          for (const ln of lines) {
            const nm = String(ln.name || '').trim().toLowerCase()
            const qty = Number(ln.qty || ln.quantity || 0)
            if (!ref || !nm) continue
            const key = ref + '|' + nm
            idx[key] = (idx[key] || 0) + (isNaN(qty) ? 0 : qty)
            if (!namesByInv[ref]) namesByInv[ref] = new Set<string>()
            namesByInv[ref].add(String(ln.name || '').trim())
          }
        }
        if (mounted) {
          setReturnsIndex(idx)
          const namesObj: Record<string, string[]> = {}
          Object.keys(namesByInv).forEach(inv => { namesObj[inv] = Array.from(namesByInv[inv]) })
          setReturnNamesByInvoice(namesObj)
        }
      } catch {
        if (mounted) {
          setReturnsIndex({})
          setReturnNamesByInvoice({})
        }
      }
    })()
  ...
  const exportCsv = async () => {
    try {
      ...
      const toRow = (p: any) => {
        ...
        const retKey = String(p.invoice || '').trim() + '|' + String(l.name || '').trim().toLowerCase()
        const retQty = returnsIndex[retKey] || 0
        const totalItems = Math.max(0, Number(origTotal) - Number(retQty || 0))
        const nameFallback = (() => {
          const inv = String(p.invoice || '').trim()
          const arr = returnNamesByInvoice[inv] || []
          return arr.length ? arr.join(', ') : '-'
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="grid gap-3 md:grid-cols-5">
          ...
          <div className="md:col-span-2">
            ...
            <input id="pharmacy-purchase-history-search" value={search} onChange={e=>setSearch(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="medicine, supplier, invoice" />
          </div>
      ...
      {/* Total Purchase Card */}
      <div className="rounded-xl border border-slate-200 bg-gradient-to-r from-blue-50 to-white p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-slate-600">Total Purchase</div>
            <div className="text-2xl font-bold text-blue-700">Rs {totalPurchaseAmount.toFixed(2)}</div>
          </div>
        ...
        {(from || to) && (
          <div className="mt-2 text-xs text-slate-500">
            {from && to ? `Period: ${from} to ${to}` : from ? `From: ${from}` : `To: ${to}`}
          </div>
        )}
      </div>
