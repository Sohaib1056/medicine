...
export default function Pharmacy_Returns() {
  ...
  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-customer-return-invoice') as HTMLInputElement | null)?.focus() }, 0)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setLoading(true)
      try {
        const res: any = await pharmacyApi.listSales({ 
          bill: invoiceId || undefined, 
          customer: customer || undefined, 
          medicine: medicineName || undefined,
          from: from || undefined, 
          to: to || undefined, 
          page, 
          limit 
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="grid gap-3 md:grid-cols-7 items-end">
          <div>
            ...
            <input id="pharmacy-customer-return-invoice" value={invoiceId} onChange={e=>setInvoiceId(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="Exact bill/invoice" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={medicineName} onChange={e=>setMedicineName(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="Search by medicine" />
          </div>
          <div>
