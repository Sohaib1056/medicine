...
export default function Pharmacy_ReturnHistory() {
  ...
  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-return-history-search') as HTMLInputElement | null)?.focus() }, 0)
    ...
    ;(async () => {
      ...
      try {
        ...
        if (mounted) {
          ...
          // Calculate total return amount from filtered results
          const totalAmount = mapped.reduce((sum: number, row: Row) => sum + (row.total || 0), 0)
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="grid gap-3 md:grid-cols-5">
          <div className="md:col-span-2">
            ...
            <input id="pharmacy-return-history-search" value={search} onChange={e=>setSearch(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="invoice, supplier, medicine" />
          </div>
          <div>
      ...
      {/* Total Return Card */}
      <div className="rounded-xl border border-slate-200 bg-gradient-to-r from-orange-50 to-white p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-slate-600">Total Return</div>
            <div className="text-2xl font-bold text-orange-700">Rs {totalReturnAmount.toFixed(2)}</div>
          </div>
