...
export default function Pharmacy_Referrals(){
  ...
  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-referrals-search') as HTMLInputElement | null)?.focus() }, 0)
  ...
  useEffect(() => {
    const onSale = () => { load() }
    window.addEventListener('pharmacy:sale' as any, onSale as any)
    return () => window.removeEventListener('pharmacy:sale' as any, onSale as any)
  }, [load])
    ...
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <input id="pharmacy-referrals-search" value={q} onChange={e=>setQ(e.target.value)} placeholder="Search" className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
        </div>
      </div>
