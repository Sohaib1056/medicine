...
export default function Pharmacy_SupplierReturns() {
  ...
  useEffect(() => {
    const onRefresh = () => setSearchTick(t => t + 1)
    window.addEventListener('pharmacy:sup-ret:refresh', onRefresh)
    return () => window.removeEventListener('pharmacy:sup-ret:refresh', onRefresh)
  }, [])

  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-supplier-return-supplier') as HTMLInputElement | null)?.focus() }, 0)
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="grid items-end gap-3 md:grid-cols-6">
          <div>
            ...
            <input id="pharmacy-supplier-return-supplier" value={supplier} onChange={e=>setSupplier(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="e.g., zain" />
          </div>
          <div>
