...
export default function Pharmacy_SidebarPermissions() {
  ...
  // Flatten leaf nav items (those with a concrete 'to')
  function flattenNav(items: Array<{ to?: string; label: string; Icon?: any; children?: any[] }>): Array<{ to: string; label: string }> {
    const out: Array<{ to: string; label: string }> = []
    for (const it of items) {
      if (it.children && Array.isArray(it.children)) {
        out.push(...flattenNav(it.children as any))
      } else if (it.to) {
        out.push({ to: String(it.to), label: String(it.label) })
      }
    }
    // Filter out staff-related pages
    return out.filter(item => !item.to.includes('/staff-'))
  }
  ...
  const allowedPaths = (() => {
    ...
    for (const item of flattenNav(pharmacySidebarNav as any)) {
  ...
  const normalizePermissions = (perms: Permission[]) => {
    ...
    const leafNav = flattenNav(pharmacySidebarNav as any)
    const allowedNav = allowedPaths ? leafNav.filter(i => allowedPaths.has(i.to)) : leafNav
