...
export default function Pharmacy_AuditLogs() {
  ...
  function parseAdjustment(detail?: string){
    const d = String(detail || '')
    const out: { item?: string; old?: number; next?: number; delta?: number; reason?: string } = {}
            ...
            <div key={l.id} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  {l.action === 'Inventory Adjustment' && (() => {
                    const p = parseAdjustment(l.detail)
                    return (
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
                        {p.item && <span className="rounded bg-indigo-50 px-2 py-0.5 text-indigo-700">{p.item}</span>}
                        {p.reason && <span className="rounded bg-amber-50 px-2 py-0.5 text-amber-700">{p.reason}</span>}
                        {p.delta != null && <span className={`rounded px-2 py-0.5 ${Number(p.delta) < 0 ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>Δ {Number(p.delta)}</span>}
                      </div>
                    )
                  })()}
                </div>
