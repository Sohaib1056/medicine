...
export default function Pharmacy_ManagerCashCount(){
  ...
  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-manager-cashcount-date') as HTMLInputElement | null)?.focus() }, 0)
      ...
      <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3 sm:p-4">
        <div className="grid gap-2 sm:grid-cols-6">
          <div>
            ...
            <input id="pharmacy-manager-cashcount-date" type="date" value={date} onChange={e=>setDate(e.target.value)} className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900" />
          </div>
          <div>
