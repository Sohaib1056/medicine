...
export default function Pharmacy_Reports() {
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="breakdown">
                ...
                <div class="row" style="background: white;">
                  <span class="row-label">Total Stock Value (Purchase)</span>
                  <span class="row-value">PKR ${summary.stockValuePurchase.toLocaleString()}</span>
                </div>
                <div class="row" style="background: white;">
                  <span class="row-label">Total Stock Value (Sale)</span>
                  <span class="row-value">PKR ${summary.stockValueSale.toLocaleString()}</span>
                </div>
              </div>
            </div>
            <div class="footer">
  ...
  useEffect(() => {
    ...
    const onPrint = () => printClosingSheet()
    const onDownload = () => {
      const text = [
        `====================================`,
        ...
        `Stock Value (Pur):PKR ${summary.stockValuePurchase.toLocaleString()}`,
        `Stock Value (Sale):PKR ${summary.stockValueSale.toLocaleString()}`,
        ``,
        `====================================`,
    ...
    window.addEventListener('pharmacy:reports:refresh', onRefresh)
    window.addEventListener('pharmacy:reports:print', onPrint)
    window.addEventListener('pharmacy:reports:download', onDownload)

    return () => {
      window.removeEventListener('pharmacy:reports:refresh', onRefresh)
      window.removeEventListener('pharmacy:reports:print', onPrint)
      window.removeEventListener('pharmacy:reports:download', onDownload)
    }
  }, [summary, from, to, pharmacyName])
    ...
    async function load() {
      try {
        ...
        setSummary({
          ...
          stockValueSale: Number(stats?.stockSaleValue || 0),
          stockValuePurchase: Number(stats?.stockPurchaseValue || 0),
        })
        setComparison([
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        ...
        <SummaryCard title="Total Stock Value (Purchase)" value={`PKR ${summary.stockValuePurchase.toLocaleString()}`} bg="bg-yellow-50" border="border-yellow-200" />
        <SummaryCard title="Total Stock Value (Sale)" value={`PKR ${summary.stockValueSale.toLocaleString()}`} bg="bg-teal-50" border="border-teal-200" />
        ...
        <div className={innerPanelClass}>
          ...
          <div className="space-y-3">
            ...
            <div className="mt-4 rounded-xl bg-slate-50 p-4 dark:bg-slate-800/50">
              <div className="mb-3 text-xs font-bold uppercase tracking-wide text-slate-500">Sales Breakdown</div>
              <div className="space-y-2">
                <div className="flex items-center justify-between rounded-lg bg-white p-3">
                  <span className="text-sm font-medium text-slate-600 ">💵 Cash Sales</span>
                  <span className="font-bold text-slate-900 ">PKR {summary.cashSales.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-white p-3">
                  <span className="text-sm font-medium text-slate-600">� Stock Value (Purchase)</span>
                  <span className="font-bold text-slate-900">PKR {summary.stockValuePurchase.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-white p-3">
                  <span className="text-sm font-medium text-slate-600">📊 Stock Value (Sale)</span>
                  <span className="font-bold text-slate-900">PKR {summary.stockValueSale.toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
