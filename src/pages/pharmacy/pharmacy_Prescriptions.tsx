...
export default function Pharmacy_Prescriptions(){
  ...
  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-prescription-id') as HTMLInputElement | null)?.focus() }, 0)
    ...
    <div className="space-y-4 p-6">
      ...
      <form onSubmit={open} className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="flex items-center gap-2">
          <input id="pharmacy-prescription-id" value={id} onChange={e=>setId(e.target.value)} placeholder="Prescription ID" className="w-full max-w-md rounded-md border border-slate-300 px-3 py-2 text-sm" />
