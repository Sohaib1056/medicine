...
export default function Pharmacy_Customers() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('pharmacy:customers:add', onAdd)
    window.addEventListener('pharmacy:customers:export', onExport)

    return () => {
      window.removeEventListener('pharmacy:customers:add', onAdd)
      window.removeEventListener('pharmacy:customers:export', onExport)
    }
  }, [])
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button ref={addButtonRef} onClick={() => setAddOpen(true)} className="btn"><Plus className="h-4 w-4" /> Add Customer</button>
          </div>
        </div>
