...
export default function Pharmacy_SalesHistory() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('pharmacy:sales:export-csv', onExportCsv)
    window.addEventListener('pharmacy:sales:export-pdf', onExportPdf)

    return () => {
      window.removeEventListener('pharmacy:sales:export-csv', onExportCsv)
      window.removeEventListener('pharmacy:sales:export-pdf', onExportPdf)
    }
  }, [rows])

  useEffect(() => {
    const t = setTimeout(() => { (document.getElementById('pharmacy-sales-history-medicine') as HTMLInputElement | null)?.focus() }, 0)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Calculate total sale amount from filtered results
        const totalAmount = mapped.reduce((sum: number, row: Row) => sum + (row.amount || 0), 0)
  ...
  return (
    <>
      <div className="space-y-4 p-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="grid gap-3 md:grid-cols-5">
            <div>
              ...
              <input id="pharmacy-sales-history-medicine" value={medicine} onChange={e=>setMedicine(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="e.g., Paracetamol" />
            </div>
            <div>
        ...
        {/* Total Sale Card */}
        <div className="rounded-xl border border-slate-200 bg-gradient-to-r from-emerald-50 to-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm text-slate-600">Total Sale</div>
              <div className="text-2xl font-bold text-emerald-700">Rs {totalSaleAmount.toFixed(2)}</div>
            </div>
      ...
      <Pharmacy_POSReceiptDialog
        ...
        lines={(receiptSale?.lines || []).map((l:any)=> ({ 
          name: l.name, 
          qty: l.qty, 
          price: l.unitPrice || 0,
          discountRs: l.discountRs || 0,
          lineTaxType: l.lineTaxType || 'percent',
          lineTaxValue: l.lineTaxValue || 0,
          packMode: l.packMode || 'loose'
        }))}
        discountPct={receiptSale?.discountPct || 0}
        lineDiscountRs={(receiptSale?.lines || []).reduce((s:number, l:any) => s + (l.discountRs || 0), 0)}
        // Use saved calculated values if available, otherwise receipt will calculate
        lineGstSum={receiptSale?.lineGstTotal}
        billGstType={receiptSale?.billGstType || 'percent'}
        billGstValue={Number(receiptSale?.billGstValue) || 0}
        billGstAmount={receiptSale?.billGstAmount}
