        ...
        <div className="mt-3 overflow-x-auto text-sm">
          <table className="min-w-full">
            <thead className="bg-slate-50 text-slate-700">
              <tr>
                <th className="px-2 py-2 text-left">DateTime</th>
            ...
            <tbody className="divide-y">
              {list.length===0 ? (
                <tr><td colSpan={6} className="px-2 py-6 text-center text-slate-500">{loading ? 'Loading...' : 'No ER tokens found'}</td></tr>
              ) : list.map(r => (
                <tr key={r.id}>
                  ...
                  <div className="pt-2">
                    ...
                    <div className="mt-2 overflow-x-auto">
                      <table className="min-w-full text-sm">
                        <thead className="bg-slate-50 text-slate-700"><tr><th className="px-2 py-1 text-left">Date</th><th className="px-2 py-1 text-left">Method</th><th className="px-2 py-1 text-right">Amount</th></tr></thead>
                        <tbody className="divide-y">
                          {payments.length===0 ? <tr><td colSpan={3} className="px-2 py-4 text-center text-slate-500">None</td></tr> : payments.map((p:any)=>(
                            <tr key={String(p._id||p.id)}>
                              <td className="px-2 py-1">{new Date(p.receivedAt||p.createdAt||new Date().toISOString()).toLocaleString()}</td>
