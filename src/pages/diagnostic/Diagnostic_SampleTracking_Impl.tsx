...
export default function Diagnostic_SampleTracking(){
  ...
  useEffect(()=>{ let mounted = true; (async()=>{
    try {
      ...
      let items: Order[] = (res.items||[]).map((x:any)=>({ id: String(x._id), createdAt: x.createdAt || new Date().toISOString(), patient: x.patient || { fullName: '-', phone: '' }, tests: x.tests || [], items: x.items || [], status: x.status || 'received', tokenNo: x.tokenNo, sampleTime: x.sampleTime, subtotal: Number(x.subtotal||0), discount: Number(x.discount||0), net: Number(x.net||0) }))
  ...
  const requestDeleteItem = async (orderId: string, testId: string) => {
    setConfirmDelete({ orderId, testId })
  }
  const doConfirmDelete = async () => {
    if (!confirmDelete) return
    const { orderId, testId } = confirmDelete
  ...
  const [confirmDelete, setConfirmDelete] = useState<{orderId: string, testId: string} | null>(null)
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      ...
      <ConfirmDialog
        ...
        message="Delete this test from the order?"
        confirmText="Delete"
