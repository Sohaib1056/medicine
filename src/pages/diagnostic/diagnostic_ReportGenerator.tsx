...
export default function Diagnostic_ReportGenerator(){
  ...
  function printHtmlWithDiagnosticChrome(input: {
    ...
    const onKey = (e: KeyboardEvent)=> {
      ...
      if ((e.ctrlKey||e.metaKey) && (e.key==='p' || e.key==='P')) { e.preventDefault(); void onPrint() }
  ...
  async function printItem(r: Result){
    ...
    // Template registry removed - using local print function instead
    ...
    // if (!tpl || !tpl.print){
      try {
        ...
        setToast({ type: 'error', message: 'No report template mapped for this test. Please set mapping in Diagnostic Settings.' })
      }
      return
    // }
  ...
  async function deleteItem(r: Result){
    setConfirmDeleteId(String(r.id))
  }
  ...
  async function confirmDelete(){
    ...
    if (!id) return
    try {
      await diagnosticApi.deleteResult(id)
