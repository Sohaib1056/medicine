...
export default function Diagnostic_ResultEntry(){
  ...
  useEffect(()=>{ let mounted = true; (async()=>{
    try {
      ...
      let items: Order[] = (res.items||[]).map((x:any)=>({ id: String(x._id), tokenNo: x.tokenNo, createdAt: x.createdAt, patient: x.patient, tests: x.tests||[], referringConsultant: x.referringConsultant, items: x.items||[], status: x.status }))
  ...
  })(); return ()=>{ mounted=false } }, [q, from, to, status, priority, page, rows])
  ...
  async function save(){
    ...
    // Clear selection
    closeDialog()
    setToast({ type: 'success', message: 'Result finalized' })
  }
    ...
    <div className="space-y-4 p-4 md:p-6">
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
          ...
          <div className="min-w-0 w-full">
            <input type="date" value={from} onChange={e=>{ setFrom(e.target.value); setPage(1) }} className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none" placeholder="From" />
          </div>
          <div className="min-w-0 w-full">
            <input type="date" value={to} onChange={e=>{ setTo(e.target.value); setPage(1) }} className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none" placeholder="To" />
          </div>
          <div className="min-w-0 w-full">
            <select value={status} onChange={e=>{ setStatus(e.target.value as any); setPage(1) }} className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none">
          ...
          <div className="min-w-0 w-full">
            ...
            <select value={priority} onChange={e=>{ setPriority(e.target.value as any); setPage(1) }} className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none">
          ...
          <div className="min-w-0 w-full">
            <select value={rows} onChange={e=>{ setRows(Number(e.target.value)); setPage(1) }} className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none">
