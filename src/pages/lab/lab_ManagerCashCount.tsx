...
export default function Pharmacy_ManagerCashCount(){
  ...
  const [expected, setExpected] = useState<{ Cash: number; Card: number; Bank: number; Online: number; Total: number }>({ Cash: 0, Card: 0, Bank: 0, Online: 0, Total: 0 })
  ...
  async function fetchExpected(){
    try{
      const effFrom = (from || date || todayStr())
      const effTo = (to || date || todayStr())
      const r: any = await labApi.reportsSummary({ from: effFrom, to: effTo })
      const p = r?.paymentsByMethod || {}
      const Cash = Number(p?.Cash || 0)
      const Card = Number(p?.Card || 0)
      const Bank = Number(p?.Bank || 0)
      const Online = Number(p?.Online || 0)
      const Total = Number((Cash + Card + Bank + Online).toFixed(2))
      setExpected({ Cash, Card, Bank, Online, Total })
    } catch {
      setExpected({ Cash: 0, Card: 0, Bank: 0, Online: 0, Total: 0 })
    }
  }
  useEffect(()=>{ fetchExpected() }, [from, to, date])

  const add = async () => {
    const amt = parseFloat(String(amount||'').trim())
    if (!isFinite(amt) || amt <= 0) { showToast('error', 'Enter a valid amount'); return }
    let created: any
    try {
    ...
    } catch (e) { showToast('error', 'Failed to save'); return }
  ...
  return (
    <div className="w-full p-3 sm:p-4">
      ...
      <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3 sm:p-4">
        <div className="text-sm font-medium text-slate-800">Expected Collections (from Invoices)</div>
        <div className="mt-2 grid gap-2 sm:grid-cols-5 text-sm">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="text-xs text-slate-600">Cash</div>
            <div className="mt-1 font-semibold">PKR {expected.Cash.toFixed(2)}</div>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="text-xs text-slate-600">Card</div>
            <div className="mt-1 font-semibold">PKR {expected.Card.toFixed(2)}</div>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="text-xs text-slate-600">Bank</div>
            <div className="mt-1 font-semibold">PKR {expected.Bank.toFixed(2)}</div>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="text-xs text-slate-600">Online</div>
            <div className="mt-1 font-semibold">PKR {expected.Online.toFixed(2)}</div>
          </div>
          <div className="rounded-lg border border-slate-200 bg-emerald-50 p-3">
            <div className="text-xs text-slate-600">Total</div>
            <div className="mt-1 font-semibold">PKR {expected.Total.toFixed(2)}</div>
          </div>
        </div>
        <div className="mt-2 text-xs text-slate-500">Use From/To filters below to match cashier shift/date window, then reconcile with cash count entries.</div>
      </div>
