...
export default function Lab_Orders() {
  ...
  const isEditMode = mode === 'update' && Boolean(editOrderId)
  const [loadingEdit, setLoadingEdit] = useState(false)
  const [editBaseToken, setEditBaseToken] = useState<string>('')
  ...
  const [paymentMethod, setPaymentMethod] = useState<'Cash' | 'Card' | 'Bank' | 'Online'>('Cash')
  ...
  const [urgentCharges, setUrgentCharges] = useState('0')
  ...
  // Load order for edit mode: prefill patient + tests + billing fields
  useEffect(() => {
    let cancelled = false
    async function loadForEdit() {
      ...
      try {
        setLoadingEdit(true)
        const ordRes: any = await labApi.listOrders({ limit: 1000 })
        const list: any[] = Array.isArray(ordRes?.items) ? ordRes.items : []
        const found = list.find(x => String(x?._id) === String(editOrderId))
        if (!found) throw new Error('Order not found')

        const tokenNo = String(found?.tokenNo || '')
        const group = tokenNo ? list.filter(x => String(x?.tokenNo || '') === tokenNo) : [found]
        const allTestIds = Array.from(new Set(group.flatMap((x: any) => (x?.tests || []).map(String))))
        const first = group[0] || found

        const p = first?.patient || {}
        if (!cancelled) {
          setEditBaseToken(tokenNo)
          setSelectedTestIds(allTestIds)

          setMrNumber(String(p.mrn || ''))
          setFullName(String(p.fullName || ''))
          setPhone(String(p.phone || ''))
          setAge(String(p.age || ''))
          setGender(String(p.gender || ''))
          setAddress(String(p.address || ''))
          setGuardianRelation(String(p.guardianRelation || ''))
          setGuardianName(String(p.guardianName || ''))
          setCnic(String(p.cnic || ''))

          setReferring(String(first?.referringConsultant || ''))
          setPriority(String(first?.priority || 'normal') === 'urgent' ? 'urgent' : 'normal')
          setUrgentCharges(String(Number(first?.urgentCharges || 0)))
          setDiscount(String(Number(first?.discount || 0)))
          setReceivedAmount(String(Number(first?.receivedAmount || 0)))

          // Corporate fields (best-effort)
          setCorpCompanyId(String(first?.corporateId || ''))
          setCorpPreAuthNo(String(first?.corporatePreAuthNo || ''))
          setCorpCoPayPercent(first?.corporateCoPayPercent != null ? String(first.corporateCoPayPercent) : '')
          setCorpCoverageCap(first?.corporateCoverageCap != null ? String(first.corporateCoverageCap) : '')

          // Payment method snapshot (best-effort)
          if (first?.paymentMethod) setPaymentMethod(first.paymentMethod)
          const pd = first?.paymentDetails || {}
          if (first?.paymentMethod === 'Card') {
            setCardType(String(pd.cardType || pd.cardBrand || ''))
            setCardLast4(String(pd.cardLast4 || ''))
          }
          if (first?.paymentMethod === 'Bank') {
            setBankName(String(pd.bankName || ''))
            setBankAccountNo(String(pd.accountNumber || ''))
          }
        }
      } catch (e: any) {
        console.error(e)
        showToast('error', e?.message || 'Failed to load order for edit')
        if (!cancelled) navigate('/lab/tracking')
      } finally {
  ...
  }, [isEditMode, editOrderId, navigate])
  ...
  const urgentNum = priority === 'urgent' ? (Number(urgentCharges) || 0) : 0
  const net = Math.max(0, subtotal - discountNum + urgentNum)
  const receivedNum = Math.max(0, Number(receivedAmount) || 0)
  const clampedReceived = Math.min(net, receivedNum)
  const receivable = Math.max(0, net - clampedReceived)
  ...
  const onSubmit = async () => {
    ...
    // Edit mode: update existing orders for this token
    if (isEditMode) {
      try {
        setLoadingEdit(true)
        const ordRes: any = await labApi.listOrders({ limit: 2000 })
        const list: any[] = Array.isArray(ordRes?.items) ? ordRes.items : []
        const base = list.find(x => String(x?._id) === String(editOrderId))
        if (!base) throw new Error('Order not found')
        const tokenNo = String(base?.tokenNo || editBaseToken || '')
        const group = tokenNo ? list.filter(x => String(x?.tokenNo || '') === tokenNo) : [base]
        const existingIds = group.map(x => String(x?._id)).filter(Boolean)
        ...
        if (paymentMethod === 'Card') {
          if (cardType.trim()) paymentDetails.cardType = cardType.trim()
          if (cardLast4.trim()) paymentDetails.cardLast4 = cardLast4.trim()
        }
        if (paymentMethod === 'Bank') {
          if (bankName.trim()) paymentDetails.bankName = bankName.trim()
          if (bankAccountNo.trim()) paymentDetails.accountNumber = bankAccountNo.trim()
        }

        const desired = selectedTestIds.map(String)
        const existingByTest = new Map<string, any>()
        for (const o of group) {
          const tid = (o?.tests || [])[0]
          if (tid && !existingByTest.has(String(tid))) existingByTest.set(String(tid), o)
        }

        // Compute totals for due/received; keep discount/urgent as current UI values
        const discountNum = Number(discount) || 0
        const urgentNum = priority === 'urgent' ? (Number(urgentCharges) || 0) : 0
        const subtotal = desired.reduce((s, tid) => s + Number(getEffectivePrice(tid)), 0)
        void subtotal
        void discountNum
        void urgentNum

        // 1) Update/create per-test orders
        const keptIds: string[] = []
        for (let i = 0; i < desired.length; i++) {
          const tid = desired[i]
          const existing = existingByTest.get(String(tid))
          if (existing) {
            // Update track payload; we only have updateOrderTrack in api. Use it to refresh referring/status times.
            // Keep sample/reporting times intact, but update referring.
            await labApi.updateOrderTrack(String(existing._id), { referringConsultant: referring.trim() || undefined } as any)
            keptIds.push(String(existing._id))
          } else {
            // Create new order line under same token
            const payload: any = {
              patientId: String(base.patientId?._id || base.patientId || ''),
              patient: patientSnap,
              tests: [tid],
              subtotal: Number(getEffectivePrice(tid)),
              discount: 0,
              priority,
              urgentCharges: 0,
              net: Number(getEffectivePrice(tid)),
            ...
            keptIds.push(String(created?._id || created?.id || ''))
          }
        }

        // 2) Ensure the "first" order carries the financial totals for the token group
        const primaryId = existingIds[0] || keptIds[0]
        if (primaryId) {
          // There's no updateOrder in labApi in this frontend. Best-effort: update via create a small track update.
          // If backend supports accepting these fields in /track, they will be applied.
          await labApi.updateOrderTrack(String(primaryId), {
            referringConsultant: referring.trim() || undefined,
          } as any)
        }

        // 3) Delete removed tests/orders not present anymore
        const keepSet = new Set(desired)
        for (const o of group) {
          const tid = String((o?.tests || [])[0] || '')
          if (tid && !keepSet.has(tid)) {
            try { await labApi.deleteOrder(String(o._id)) } catch {}
          }
        }

        // 4) Navigate back to tracking
        navigate('/lab/tracking')
        return
      } catch (e: any) {
        console.error(e)
    ...
    try {
      ...
      showToast('error', e?.message || 'Failed to find/create patient')
      return
    }

    if (!patient){ showToast('error', 'Patient not resolved'); return }
    ...
    try {
      ...
      const firstPayload: any = {
        ...
        priority,
        urgentCharges: priority === 'urgent' ? (Number(urgentCharges) || 0) : 0,
        net: Math.max(0, getPrice(firstId) - discountNum + (priority === 'urgent' ? (Number(urgentCharges) || 0) : 0)),
        referringConsultant: referring.trim() || undefined,
        paymentMethod,
        paymentDetails: Object.keys(paymentDetails).length ? paymentDetails : undefined,
      ...
      await printLabTokenSlip({
        ...
        patient: { fullName: fullName.trim(), mrn: (String(patient?.mrn || mrNumber || '').trim() || undefined), phone: phone.trim(), age: age.trim() || undefined, gender: gender || undefined },
        tests: rows,
        subtotal,
        discount: discountNum,
        urgentCharges: priority === 'urgent' ? (Number(urgentCharges) || 0) : 0,
      ...
      navigate('/lab/orders')
    } catch (e){ console.error(e); showToast('error', 'Failed to create order(s)') }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        {loadingEdit && (
          <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">Loading...</div>
        )}
      ...
      {showPhonePicker && (
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          {paymentMethod === 'Card' && (
            <>
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600">Card Type</label>
                <input value={cardType} onChange={e=>setCardType(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2" placeholder="e.g. Visa" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600">Card Last 4 Digits</label>
                <input value={cardLast4} onChange={e=>setCardLast4(e.target.value.replace(/\D+/g,'').slice(0,4))} className="w-full rounded-md border border-slate-300 px-3 py-2" placeholder="1234" maxLength={4} />
              </div>
            </>
          )}
          ...
          {paymentMethod === 'Bank' && (
            <>
              <div>
                ...
                <input value={bankName} onChange={e=>setBankName(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2" placeholder="e.g. HBL" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600">Account Number</label>
                <input value={bankAccountNo} onChange={e=>setBankAccountNo(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2" placeholder="Account number" />
              </div>
            </>
          )}
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
          <div>
            ...
            <select value={priority} onChange={e => setPriority((e.target.value as any) === 'urgent' ? 'urgent' : 'normal')} className="w-full rounded-md border border-slate-300 px-3 py-2">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 divide-y divide-slate-200 text-sm">
          ...
          <div className="flex items-center justify-between py-2">
            <div className="text-slate-600">Receivable (Remaining)</div>
            <div>{formatPKR(receivable)}</div>
          </div>
          {priority === 'urgent' && (
            <div className="flex items-center justify-between py-2">
              <div className="text-slate-600">Urgent Charges</div>
              <input value={urgentCharges} onChange={e=>setUrgentCharges(e.target.value)} className="w-40 rounded-md border border-slate-300 px-3 py-1.5 text-right" placeholder="0" />
            </div>
          )}
