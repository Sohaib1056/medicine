...
export default function Lab_Appointments() {
  ...
  async function createAppointment() {
    if (!form.dateIso) { showNotice('error', 'Select date'); return }
    ...
    try {
      ...
      setForm({ dateIso: todayIso(), phone: '', patientName: '', gender: '', age: '', time: '', notes: '', testIds: [] })
      setSelectedPatient(null)
      setPhoneSuggestItems([])
      setPhoneSuggestOpen(false)
      await load()
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600">Appointment Date *</label>
            <input type="date" value={form.dateIso} onChange={e => update('dateIso', e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2" />
          </div>
          <div>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600">Appointment Time</label>
          ...
          <div className="md:col-span-3 flex items-center justify-end gap-2">
            <button type="button" onClick={() => { setForm({ dateIso: todayIso(), phone: '', patientName: '', gender: '', age: '', time: '', notes: '', testIds: [] }); setSelectedPatient(null) }} className="rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700">Clear</button>
