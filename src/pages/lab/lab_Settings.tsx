...
export default function Lab_Settings() {
  const [activeTab, setActiveTab] = useState<'lab' | 'system' | 'pathologist'>('lab')

  const sessionRole = (()=>{ try { const s = JSON.parse(localStorage.getItem('lab.session')||'{}'); return String(s?.role||'') } catch { return '' } })()
  const isPathologist = /pathologist/i.test(sessionRole)
  ...
  const [reportQrBaseUrl, setReportQrBaseUrl] = useState('')
  const [logoDataUrl, setLogoDataUrl] = useState<string | null>(null)
  const [department, setDepartment] = useState('')
  const [reportTemplate, setReportTemplate] = useState<'classic'|'tealGradient'|'modern'|'adl'>('classic')
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try {
        ...
        setReportQrBaseUrl(s.reportQrBaseUrl || '')
        setLogoDataUrl(s.logoDataUrl || null)
        setDepartment(s.department || '')
        setReportTemplate((s.reportTemplate === 'adl' ? 'adl' : (s.reportTemplate === 'tealGradient' ? 'tealGradient' : (s.reportTemplate === 'modern' ? 'modern' : 'classic'))))
  ...
  // Pathologist Settings (per-user, stored locally)
  const [pmdcNumber, setPmdcNumber] = useState<string>(()=>{ try { return localStorage.getItem('lab.pathologist.pmdc') || '' } catch { return '' } })
  const [digitalSignature, setDigitalSignature] = useState<string>(()=>{ try { return localStorage.getItem('lab.pathologist.signature') || '' } catch { return '' } })
  ...
  const savePathologist = () => {
    try {
      localStorage.setItem('lab.pathologist.pmdc', pmdcNumber)
      localStorage.setItem('lab.pathologist.signature', digitalSignature)
      setNotice('Pathologist settings saved')
      try { setTimeout(()=> setNotice(''), 2500) } catch {}
    } catch {}
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        {isPathologist && (
          <button onClick={() => setActiveTab('pathologist')} className={`rounded-md border px-3 py-1.5 text-sm ${activeTab==='pathologist' ? 'border-slate-300 bg-white text-slate-900' : 'border-transparent bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Pathologist</button>
        )}
      </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white">
          ...
          <div className="space-y-4 p-4">
            ...
            <div>
              ...
              <select value={reportTemplate} onChange={e=> setReportTemplate((e.target.value as 'classic'|'tealGradient'|'modern'|'adl') || 'classic')} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
            ...
            <div>
              <label className="mb-1 block text-sm text-slate-700">Report QR Link (Base URL)</label>
              <input value={reportQrBaseUrl} onChange={e=>setReportQrBaseUrl(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="https://your-domain.com/lab/report" />
              <div className="mt-1 text-xs text-slate-500">Scanning the QR will open this link. The system will append the token/lab number automatically.</div>
            </div>
      ...
      {activeTab === 'pathologist' && isPathologist && (
        <div className="rounded-xl border border-slate-200 bg-white">
          <div className="border-b border-slate-200 px-4 py-3 font-medium text-slate-800">Pathologist Settings</div>
          <div className="space-y-4 p-4">
            <div>
              <label className="mb-1 block text-sm text-slate-700">PMDC Number</label>
              <input value={pmdcNumber} onChange={e=>setPmdcNumber(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="e.g., 12345-P" />
            </div>

            <div>
              <label className="mb-1 block text-sm text-slate-700">Digital Signature</label>
              <input
                type="file"
                accept="image/*"
                onChange={e=>{
                  const file = e.target.files?.[0]
                  if (!file) { setDigitalSignature(''); return }
                  const reader = new FileReader()
              ...
              {digitalSignature && (
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-600">
                  <img src={digitalSignature} alt="Signature preview" className="h-12 w-auto rounded border" />
                  <span>Preview</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-end">
              <button onClick={savePathologist} className="btn">Save Settings</button>
            </div>
          </div>
        </div>
      )}
