  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mb-3 -mx-1 flex flex-nowrap gap-2 overflow-x-auto px-1 text-sm">
          {tabs.map(tag => (
            <button
              key={tag}
              onClick={()=>setActiveTab(tag)}
              className={`shrink-0 rounded-md border px-3 py-1.5 ${activeTab===tag? 'border-navy-600 bg-navy-50 text-navy-700' : 'border-slate-200 bg-slate-50 hover:bg-slate-100'}`}
            >{tag}</button>
          ))}
        </div>
