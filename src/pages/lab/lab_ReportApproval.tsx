...
type ResultRow = { id: string; test: string; normal?: string; unit?: string; prevValue?: string; value?: string; flag?: 'normal'|'low'|'high'|'critical_low'|'critical_high'|'abnormal'|'critical'; comment?: string }
      ...
      try {
        ...
        const o: Order | null = found
          ? {
              ...
              priority: String(found.priority||'normal') === 'urgent' ? 'urgent' : 'normal',
              reportingTime: found.reportingTime,
              referringConsultant: found.referringConsultant,
              recollectionStatus: found.recollectionStatus || 'none',
            }
          : null
        setOrder(o)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setList(
          orders
            .filter(o => o.recollectionStatus !== 'required')
  ...
  const preview = async () => {
    ...
    await previewLabReportPdf({
      ...
      rows: (result.rows || []).map(r => ({
        ...r,
        flag: (r.flag === 'critical_low' || r.flag === 'critical_high') ? 'critical' : r.flag,
      })) as import('../../utils/printLabReport').ReportRow[],
              ...
              {list.map(({ order, result }) => {
                ...
                return (
                  <tr key={order.id} className="border-b border-slate-100">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">{String((order as any).priority || 'normal') === 'urgent' ? 'Urgent' : 'Normal'}</td>
                    <td className="px-3 py-2 whitespace-nowrap">
                      ...
                      {(order as any).recollectionStatus === 'required' && (
                        <span className="ml-1 rounded-full px-2 py-0.5 text-xs bg-rose-100 text-rose-700">Re-collect</span>
                      )}
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {order?.recollectionStatus === 'required' && (
              <span className="rounded-full px-2 py-0.5 text-xs bg-rose-100 text-rose-700" title="Recollection required before approval">
                Re-collect Needed
              </span>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            {order?.recollectionStatus === 'required' && (
              <div className="rounded-md border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">
                <strong>Recollection Required:</strong> This sample has been marked for recollection ({order.recollectionReason || 'reason not specified'}). Report approval is blocked until recollection is completed.
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              <button
                onClick={async () => {
                  if (!result) return
                  try {
                    await labApi.updateResult(result.id, { reportStatus: 'rejected', approvedAt: undefined, approvedBy: undefined })
                    await labApi.updateOrderTrack(orderId, { status: 'received' } as any)
              ...
              <button
                onClick={async () => {
                  if (!result) return
                  if (order?.recollectionStatus === 'required') {
                    showToast('error', 'Cannot approve: Recollection is required for this sample.')
                    return
                  }
                  try {
                    ...
                    navigate(`/lab/reports?orderId=${encodeURIComponent(orderId)}`)
                  } catch (e) {
                    console.error(e)
                    showToast('error', 'Failed to approve report')
                  }
                }}
                disabled={order?.recollectionStatus === 'required'}
                className={`inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-white ${order?.recollectionStatus === 'required' ? 'bg-slate-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700'}`}
              >
