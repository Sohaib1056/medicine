...
type ResultRow = { id: string; test: string; normal?: string; unit?: string; value?: string; comment?: string; flag?: 'normal'|'low'|'high'|'critical_low'|'critical_high'|'abnormal'|'critical' }

type ResultRecord = { id: string; orderId: string; rows: ResultRow[]; interpretation?: string; createdAt: string; reportStatus?: 'pending'|'approved'|'rejected' }
...
function rowFlag(r: ResultRow) {
  if (r.flag === 'critical' || r.flag === 'critical_low' || r.flag === 'critical_high') return 'critical'
...
function asDateTimeParts(fallbackIso: string, timeOrIso?: string) {
  const raw = String(timeOrIso || '').trim()
  if (raw && /\d{4}-\d{2}-\d{2}t/i.test(raw)) {
  ...
  const date = !isNaN(base.getTime()) ? base.toLocaleDateString() : ''
  return { date, time: raw }
}

function normalizePdfFlag(flag: ResultRow['flag']){
  if (flag === 'critical_low' || flag === 'critical_high') return 'critical' as const
  if (flag === 'low' || flag === 'high') return 'abnormal' as const
  return flag as ('normal' | 'low' | 'high' | 'abnormal' | 'critical' | undefined)
}
...
export default function Lab_ReportGenerator() {
  ...
  const maybeRecoverDue = async (order: Order | undefined) => {
    try {
      if (!order) return
      const due = Math.max(0, Number(order.receivableAmount || 0))
      if (due <= 0) return
      const raw = prompt(`This token has due amount PKR ${due.toFixed(2)}.\nEnter amount to recover now (or Cancel to skip):`, String(due.toFixed(2)))
      if (raw == null) return
      const amt = Math.max(0, Number(String(raw).replace(/[^0-9.]/g, '')) || 0)
      if (!amt) return
      await labApi.collectOrderPayment(String(order.id), { amount: Math.min(due, amt), paymentMethod: 'Cash' })
    } catch (e) {
      console.error(e)
      // optional prompt only: do not block printing
    }
  }
  useEffect(() => {
    if (!orderId) return
    ...
    ;(async()=>{
      try {
        ...
        const list = (resRes.items||[])
          ...
          .map((r:any)=>({ id: r._id, orderId: r.orderId, rows: r.rows||[], interpretation: r.interpretation, createdAt: r.createdAt || new Date().toISOString(), reportStatus: r.reportStatus }))
        setResults(list)
        setTotal(Number(list.length || 0))
        setTotalPages(Number(resRes.totalPages || 1))
        const o: Order[] = (ordRes.items||[]).map((x:any)=>({ id: x._id, createdAt: x.createdAt || new Date().toISOString(), barcode: pickBarcode(x), patient: x.patient || { fullName: '-', phone: '' }, tests: x.tests||[], status: x.status || 'received', tokenNo: x.tokenNo, sampleTime: x.sampleTime, reportingTime: x.reportingTime, referringConsultant: x.referringConsultant, priority: String(x.priority||'normal') === 'urgent' ? 'urgent' : 'normal', receivedAmount: Number(x.receivedAmount||0), receivableAmount: Number(x.receivableAmount||0) }))
        setOrders(o)
  ...
  const filtered = useMemo(() => enriched.filter(e => {
    ...
    if (priority !== 'all' && String(e.order?.priority || 'normal') !== priority) return false
  ...
  const printRow = async (e: Enriched) => {
    ...
    await maybeRecoverDue(o)
    const barcode = pickBarcode(o)
    await previewLabReportPdf({
      ...
      rows: (e.r.rows||[]).map((row: ResultRow)=>({
        ...
        flag: normalizePdfFlag(row.flag) as any,
        comment: row.comment,
      })),
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select value={priority} onChange={e=>{ setPriority(e.target.value as any); setPage(1) }} className="rounded-md border border-slate-300 px-2 py-2 text-sm">
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-slate-200 bg-slate-50 text-left text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2">Collection<br/>DateTime</th>
              <th className="px-3 py-2">Reporting<br/>DateTime</th>
            ...
            {items.map((e, idx) => (
              <tr key={e.r.id} className={`border-b border-slate-100 ${e.flag==='critical'?'bg-rose-50':''}`}>
                ...
                <td className="px-3 py-2 whitespace-nowrap">
                  {(() => {
                    const p = asDateTimeParts(String(e.order?.createdAt || e.r.createdAt || ''), String(e.track?.sampleTime || ''))
                    return (
                ...
                <td className="px-3 py-2 whitespace-nowrap">
                  {(() => {
                    const p = asDateTimeParts(String(e.order?.createdAt || e.r.createdAt || ''), String(e.track?.reportingTime || ''))
                    return (
                      <div className="leading-tight">
                        <div>{p.date || '-'}</div>
                        <div className="text-xs text-slate-500">{p.time || '-'}</div>
                ...
                <td className="px-3 py-2 whitespace-nowrap">{String((e.order as any)?.priority || 'normal') === 'urgent' ? 'Urgent' : 'Normal'}</td>
                <td className="px-3 py-2 whitespace-nowrap">{Number(e.order?.receivedAmount||0).toFixed(2)}</td>
                <td className="px-3 py-2 whitespace-nowrap">{Number(e.order?.receivableAmount||0).toFixed(2)}</td>
