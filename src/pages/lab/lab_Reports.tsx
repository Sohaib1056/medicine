...
export default function Lab_Reports() {
  ...
  const [paymentsByMethod, setPaymentsByMethod] = useState<{ Cash?: number; Card?: number; Bank?: number; Online?: number }>({})
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      ...
      try{
        ...
        setPaymentsByMethod(res?.paymentsByMethod || {})
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <SummaryCard title="Revenue (Cash)" value={loading? '…' : `PKR ${(paymentsByMethod.Cash||0).toLocaleString()}`} bg="bg-slate-50" border="border-slate-200" />
        <SummaryCard title="Revenue (Card)" value={loading? '…' : `PKR ${(paymentsByMethod.Card||0).toLocaleString()}`} bg="bg-slate-50" border="border-slate-200" />
        <SummaryCard title="Revenue (Bank)" value={loading? '…' : `PKR ${(paymentsByMethod.Bank||0).toLocaleString()}`} bg="bg-slate-50" border="border-slate-200" />
        <SummaryCard title="Revenue (Online)" value={loading? '…' : `PKR ${(paymentsByMethod.Online||0).toLocaleString()}`} bg="bg-slate-50" border="border-slate-200" />
