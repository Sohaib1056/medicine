...
export async function api(path: string, init?: RequestInit) {
  ...
  if (!res.ok) {
    // Treat some 404s as a soft "not found" to avoid noisy console errors
    if (res.status === 404) {
      const p = String(path || '')
      const softRx404 =
        /\/hospital\/opd\/encounters\/[^/]+\/prescription$/.test(p) ||
        /\/hospital\/opd\/prescriptions\/by-encounter(?:\?|$)/.test(p)
      if (softRx404) {
        try {
  ...
  if (contentType.includes('application/json')) {
    ...
    try {
      const isLogin = /\/login$/.test(path) || /\/users\/login$/.test(path)
      if (isLogin && (data as any)?.token) {
        // Clear ALL stale sessions upon a fresh login to prevent privilege bleeding
        const keysToRemove = [
          'hospital.token', 'hospital.session',
          'doctor.token', 'doctor.session',
          'pharmacy.token', 'pharmacy.session', 'pharmacy.user',
          'reception.token', 'reception.session', 'reception.user',
          'lab.token', 'lab.session', 'lab.user',
          'diagnostic.token', 'diagnostic.session', 'diagnostic.user',
          'finance.token', 'finance.session',
          'aesthetic.token', 'aesthetic.session',
          'token', 'user'
        ]
...
export const pharmacyApi = {
  ...
  listInventoryManufacturers: () => api('/pharmacy/inventory/manufacturers'),
  ...
  getCustomerByPhone: (phone: string) => api(`/pharmacy/customers/by-phone/${encodeURIComponent(phone)}`),
  getCustomerHistory: (id: string) => api(`/pharmacy/customers/${id}/history`),
  ...
  approvePurchaseDraftLine: (id: string, lineId: string) => api(`/pharmacy/purchase-drafts/${id}/lines/${lineId}/approve`, { method: 'POST' }),
  rejectPurchaseDraftLine: (id: string, lineId: string) => api(`/pharmacy/purchase-drafts/${id}/lines/${lineId}`, { method: 'DELETE' }),
  deletePurchaseDraft: (id: string) => api(`/pharmacy/purchase-drafts/${id}`, { method: 'DELETE' }),

  // Purchase Orders
  listPurchaseOrders: (params?: { status?: string; supplierId?: string; companyId?: string; from?: string; to?: string; search?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.status) qs.set('status', params.status)
  ...
  receivePurchaseOrder: (id: string, partially?: boolean) => api(`/pharmacy/purchase-orders/${id}/receive`, { method: 'POST', body: JSON.stringify({ partially }) }),
  cancelPurchaseOrder: (id: string) => api(`/pharmacy/purchase-orders/${id}/cancel`, { method: 'POST' }),
  getPurchaseOrderInventorySuggestions: (q?: string, limit?: number) => {
    const qs = new URLSearchParams()
    if (q) qs.set('q', q)
    if (limit != null) qs.set('limit', String(limit))
    const s = qs.toString()
    return api(`/pharmacy/purchase-orders/inventory-suggestions${s ? `?${s}` : ''}`)
  },
  getPurchaseOrderSummary: (params?: { from?: string; to?: string; status?: string }) => {
    const qs = new URLSearchParams()
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    if (params?.status) qs.set('status', params.status)
    const s = qs.toString()
    return api(`/pharmacy/purchase-orders/summary${s ? `?${s}` : ''}`)
  },
  ...
  uploadInventoryImage: async (file: File): Promise<{ url: string }> => {
    const token = getToken('/pharmacy')
    const formData = new FormData()
    formData.append('image', file)
    const headers: Record<string, string> = {}
    if (token) headers['Authorization'] = `Bearer ${token}`
    const res = await fetch(`${baseURL}/pharmacy/inventory/upload-image`, { method: 'POST', body: formData, headers })
    if (!res.ok) {
      const text = await res.text()
      throw new Error(text || res.statusText)
    }
  ...
  // Audit Analytics (for Audit & Analytics page)
  getAuditAnalytics: (params?: { from?: string; to?: string }) => {
    const qs = new URLSearchParams()
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    const s = qs.toString()
    return api(`/pharmacy/audit/analytics${s ? `?${s}` : ''}`)
  },
  getAuditTopItems: (params?: { from?: string; to?: string; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/pharmacy/audit/top-items${s ? `?${s}` : ''}`)
  },
  getAuditSaleDetail: (id: string) => api(`/pharmacy/audit/sale/${encodeURIComponent(id)}`),
  getAuditPurchaseDetail: (id: string) => api(`/pharmacy/audit/purchase/${encodeURIComponent(id)}`),
  getAuditInventoryDetail: (id: string) => api(`/pharmacy/audit/inventory/${encodeURIComponent(id)}`),
}
...
export const labApi = {
  ...
  incomeLedger: (params?: { from?: string; to?: string; transactionType?: string; paymentStatus?: string; paymentMethod?: string; search?: string; page?: number; limit?: number; sortBy?: string; sortOrder?: string }) => {
    const qs = new URLSearchParams()
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
  ...
  searchPatients: (params?: { q?: string; phone?: string; name?: string; cnic?: string; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.phone) qs.set('phone', params.phone)
    if (params?.name) qs.set('name', params.name)
    if (params?.cnic) qs.set('cnic', params.cnic)
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/lab/patients/search${s ? `?${s}` : ''}`)
  },
  getPatientProfiling: (params: { patientId?: string; mrn?: string }) => {
    const qs = new URLSearchParams()
    if (params?.patientId) qs.set('patientId', params.patientId)
    if (params?.mrn) qs.set('mrn', params.mrn)
    return api(`/lab/patients/profiling?${qs.toString()}`)
  },
  ...
  collectOrderPayment: (id: string, data: { amount: number; paymentMethod?: 'Cash' | 'Card' | 'Bank'; paymentDetails?: any; note?: string }) =>
    api(`/lab/orders/${id}/collect-payment`, { method: 'POST', body: JSON.stringify(data) }),
...
export const hospitalApi = {
  ...
  listDepartments: async () => {
    try { return await api('/hospital/departments') }
    catch (e) {
      try {
        const raw = localStorage.getItem('hospital.fallback.departments')
        const arr = raw ? JSON.parse(raw) : []
        return { departments: arr }
      } catch { throw e }
    }
  },
  ...
  listDoctors: async (params?: { q?: string; departmentId?: string; active?: boolean; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    try { return await api(`/hospital/doctors${s ? `?${s}` : ''}`) }
    catch (e) {
      try {
        const raw = localStorage.getItem('hospital.fallback.doctors')
        const arr = raw ? JSON.parse(raw) : []
        const q = String(params?.q || '').trim().toLowerCase()
        const filtered = q ? arr.filter((t: any) => String(t?.name || '').toLowerCase().includes(q)) : arr
  ...
  listTokens: async (params?: { date?: string; from?: string; to?: string; status?: 'queued' | 'in-progress' | 'completed' | 'returned' | 'cancelled'; doctorId?: string; departmentId?: string; scheduleId?: string }) => {
    const qs = new URLSearchParams()
    if (params?.date) qs.set('date', params.date)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    ...
    if (params?.scheduleId) qs.set('scheduleId', params.scheduleId)
    const s = qs.toString()
    try { return await api(`/hospital/tokens${s ? `?${s}` : ''}`) }
    catch (e) {
      try {
        const key = `hospital.fallback.tokens.${params?.doctorId || 'all'}`
        const raw = localStorage.getItem(key) || localStorage.getItem('hospital.fallback.tokens') || '[]'
  ...
  // ER Vitals (Daily Monitoring)
  listErVitals: (encounterId: string, params?: { page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.page != null) qs.set('page', String(params.page))
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/encounters/${encodeURIComponent(encounterId)}/vitals${s ? `?${s}` : ''}`)
  },
  createErVital: (encounterId: string, data: { recordedAt?: string; bp?: string; hr?: number; rr?: number; temp?: number; spo2?: number; bsr?: number; intakeIV?: string; urine?: string; nurseSign?: string; shift?: 'morning' | 'evening' | 'night' }) =>
    api(`/hospital/er/encounters/${encodeURIComponent(encounterId)}/vitals`, { method: 'POST', body: JSON.stringify(data) }),

  // ER Clinical Notes
  listErClinicalNotes: (encounterId: string, params?: { type?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.type) qs.set('type', params.type)
    if (params?.page != null) qs.set('page', String(params.page))
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/encounters/${encodeURIComponent(encounterId)}/clinical-notes${s ? `?${s}` : ''}`)
  },
  createErClinicalNote: (encounterId: string, data: { type: string; recordedAt?: string; createdBy?: string; createdByRole?: string; doctorName?: string; sign?: string; data?: any }) =>
    api(`/hospital/er/encounters/${encodeURIComponent(encounterId)}/clinical-notes`, { method: 'POST', body: JSON.stringify(data) }),

  // ER Medication Orders
  listErMedOrders: (encounterId: string, params?: { page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.page != null) qs.set('page', String(params.page))
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/encounters/${encodeURIComponent(encounterId)}/med-orders${s ? `?${s}` : ''}`)
  },
  createErMedOrder: (encounterId: string, data: { drugId?: string; drugName?: string; dose?: string; route?: string; frequency?: string; duration?: string; startAt?: string; endAt?: string; prn?: boolean; status?: 'active' | 'stopped'; prescribedBy?: string }) =>
    api(`/hospital/er/encounters/${encodeURIComponent(encounterId)}/med-orders`, { method: 'POST', body: JSON.stringify(data) }),

  updateTokenStatus: (id: string, status: 'queued' | 'in-progress' | 'completed' | 'returned' | 'cancelled') =>
    api(`/hospital/tokens/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  updateToken: (id: string, data: { discount?: number; doctorId?: string; departmentId?: string; patientId?: string; mrn?: string; patientName?: string; phone?: string; gender?: string; guardianRel?: string; guardianName?: string; cnic?: string; address?: string; age?: string; overrideFee?: number; vitals?: any }) =>
  ...
  getIPDAdmissionById: (id: string) => api(`/hospital/ipd/admissions/${encodeURIComponent(id)}`),
  getIPDAdmissionByIdSilent: async (id: string) => {
    try {
      return await api(`/hospital/ipd/admissions/${encodeURIComponent(id)}`)
    } catch {
      return null
    }
  },

  // ER: Discharge Documents
  getErDischargeSummary: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/discharge-summary`),
  upsertErDischargeSummary: (tokenId: string, data: { diagnosis?: string; courseInHospital?: string; procedures?: string[]; conditionAtDischarge?: string; medications?: string[]; advice?: string; followUpDate?: string; notes?: string; createdBy?: string }) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/discharge-summary`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErDischargeSummary: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/discharge-summary`, { method: 'DELETE' }),
  // ER Death Certificate
  getErDeathCertificate: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/death-certificate`),
  upsertErDeathCertificate: (tokenId: string, data: {
    ...
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/death-certificate`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErDeathCertificate: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/death-certificate`, { method: 'DELETE' }),
  // ER Birth Certificate
  getErBirthCertificate: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/birth-certificate`),
  upsertErBirthCertificate: (tokenId: string, data: any) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/birth-certificate`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErBirthCertificate: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/birth-certificate`, { method: 'DELETE' }),
  // ER Standalone Birth Certificate (no token required)
  createErBirthCertificate: (data: any) =>
    api(`/hospital/er/forms/birth-certificates`, { method: 'POST', body: JSON.stringify(data) }),
  getErBirthCertificateById: (id: string) =>
    api(`/hospital/er/forms/birth-certificates/${encodeURIComponent(id)}`),
  updateErBirthCertificateById: (id: string, data: any) =>
    api(`/hospital/er/forms/birth-certificates/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErBirthCertificateById: (id: string) =>
    api(`/hospital/er/forms/birth-certificates/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  // ER Received Death
  getErReceivedDeath: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/received-death`),
  upsertErReceivedDeath: (tokenId: string, data: {
    ...
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/received-death`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErReceivedDeath: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/received-death`, { method: 'DELETE' }),
  // ER Short Stay
  getErShortStay: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/short-stay`),
  upsertErShortStay: (tokenId: string, data: { admittedAt?: string; dischargedAt?: string; data?: any; notes?: string; createdBy?: string }) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/short-stay`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErShortStay: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/short-stay`, { method: 'DELETE' }),

  // ER Forms Lists (for standalone pages)
  listErReceivedDeaths: (params?: { q?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/forms/received-deaths${s ? `?${s}` : ''}`)
  },
  listErDeathCertificates: (params?: { q?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/forms/death-certificates${s ? `?${s}` : ''}`)
  },
  listErBirthCertificates: (params?: { q?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/forms/birth-certificates${s ? `?${s}` : ''}`)
  },
  listErShortStays: (params?: { q?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/forms/short-stays${s ? `?${s}` : ''}`)
  },
  listErDischargeSummaries: (params?: { q?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/forms/discharge-summaries${s ? `?${s}` : ''}`)
  },
  // ER Final Invoice
  getErFinalInvoice: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/final-invoice`),
  upsertErFinalInvoice: (tokenId: string, data: any) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/final-invoice`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErFinalInvoice: (tokenId: string) =>
    api(`/hospital/er/tokens/${encodeURIComponent(tokenId)}/final-invoice`, { method: 'DELETE' }),
  // ER Standalone Final Invoice (no token required)
  createErFinalInvoice: (data: any) =>
    api(`/hospital/er/forms/final-invoices`, { method: 'POST', body: JSON.stringify(data) }),
  getErFinalInvoiceById: (id: string) =>
    api(`/hospital/er/forms/final-invoices/${encodeURIComponent(id)}`),
  updateErFinalInvoiceById: (id: string, data: any) =>
    api(`/hospital/er/forms/final-invoices/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteErFinalInvoiceById: (id: string) =>
    api(`/hospital/er/forms/final-invoices/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  listErFinalInvoices: (params?: { q?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/er/forms/final-invoices${s ? `?${s}` : ''}`)
  },
  ...
  listIpdVitals: (encounterId: string, params?: { page?: number; limit?: number }) => {
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/ipd/admissions/${encodeURIComponent(encounterId)}/vitals${s ? `?${s}` : ''}`)
  },
  createIpdVital: (encounterId: string, data: { recordedAt?: string; bp?: string; hr?: number; rr?: number; temp?: number; spo2?: number; height?: number; weight?: number; painScale?: number; recordedBy?: string; note?: string; shift?: 'morning' | 'evening' | 'night'; bsr?: number; intakeIV?: string; urine?: string; nurseSign?: string }) =>
    api(`/hospital/ipd/admissions/${encodeURIComponent(encounterId)}/vitals`, { method: 'POST', body: JSON.stringify(data) }),
  ...
  listIpdBillingItems: (encounterId: string, params?: { page?: number; limit?: number }) => {
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/hospital/ipd/admissions/${encodeURIComponent(encounterId)}/billing/items${s ? `?${s}` : ''}`)
  },
  createIpdBillingItem: (encounterId: string, data: { type: 'bed' | 'procedure' | 'medication' | 'service'; description: string; qty?: number; unitPrice?: number; amount?: number; date?: string; refId?: string; billedBy?: string }) =>
    api(`/hospital/ipd/admissions/${encodeURIComponent(encounterId)}/billing/items`, { method: 'POST', body: JSON.stringify(data) }),
  ...
  listPrescriptions: async (params?: { doctorId?: string; patientMrn?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.doctorId) qs.set('doctorId', params.doctorId)
    if (params?.patientMrn) qs.set('patientMrn', params.patientMrn)
    if (params?.from) qs.set('from', params.from)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    try { return await api(`/hospital/opd/prescriptions${s ? `?${s}` : ''}`) }
    catch (e) {
      try {
        const key = `hospital.fallback.prescriptions.${params?.doctorId || 'all'}`
        const raw = localStorage.getItem(key) || localStorage.getItem('hospital.fallback.prescriptions') || '[]'
        const arr = JSON.parse(raw)
        return { prescriptions: arr, total: arr.length, page: 1, limit: Number(params?.limit || arr.length || 50) }
      } catch { throw e }
    }
  },
  ...
  // Prescription helpers by encounter
  getPrescriptionByEncounter: async (encounterId: string) => {
    const id = encodeURIComponent(encounterId)
    const attempts: Array<{ path: string; init?: RequestInit; transform?: (r: any) => any }> = [
      { path: `/hospital/opd/encounters/${id}/prescription`, transform: (r: any) => r },
      { path: `/hospital/opd/prescriptions/by-encounter?encounterId=${id}`, transform: (r: any) => r },
      { path: `/hospital/opd/prescriptions?encounterId=${id}&limit=1`, transform: (r: any) => ({ prescription: (r?.prescriptions || r || [])[0] }) },
    ]
    let lastErr: any
    for (const a of attempts) {
      try {
        const resp = await api(a.path, a.init)
        const out = a.transform ? a.transform(resp) : resp
        if (out && typeof out === 'object' && Object.prototype.hasOwnProperty.call(out, 'prescription') && (out as any).prescription == null) {
          // Soft-not-found: try next strategy without throwing
          continue
        }
        return out
      } catch (e) { lastErr = e }
    ...
    try {
      const key = `prescriptions.byEncounter.${encounterId}`
      const raw = localStorage.getItem(key)
      const row = raw ? JSON.parse(raw) : null
      if (!row) throw lastErr || new Error('Failed to get prescription by encounter')
      return { prescription: row }
    } catch { throw lastErr || new Error('Failed to get prescription by encounter') }
  },
  upsertPrescriptionByEncounter: async (encounterId: string, data: {
    ...
    aorticSelected?: string[]
    detailsExtras?: any
    vitals?: { pulse?: number; temperatureC?: number; bloodPressureSys?: number; bloodPressureDia?: number; respiratoryRate?: number; bloodSugar?: number; weightKg?: number; heightCm?: number; bmi?: number; bsa?: number; spo2?: number }
  }) => {
    const id = encodeURIComponent(encounterId)
    const body = JSON.stringify({ ...data })
    const attempts: Array<{ path: string; init: RequestInit }> = [
      // Prefer create-first to reduce 404s from update-when-missing
      { path: `/hospital/opd/prescriptions`, init: { method: 'POST', body: JSON.stringify({ encounterId, ...data }) } },
      { path: `/hospital/opd/encounters/${id}/prescription`, init: { method: 'PUT', body } },
    ]
    let lastErr: any
    for (const a of attempts) {
      try { return await api(a.path, a.init) } catch (e) { lastErr = e }
    }
    // Fallback: try update if existing, else create
    try {
      const cur: any = await (hospitalApi as any).getPrescriptionByEncounter(encounterId)
      const presId = String(cur?.prescription?._id || cur?.prescription?.id || '')
      if (presId) return await (hospitalApi as any).updatePrescription(presId, data as any)
    } catch {}
    try { return await (hospitalApi as any).createPrescription({ encounterId, ...data } as any) } catch (e) { lastErr = e }
    // LocalStorage offline fallback
    try {
      const key = `prescriptions.byEncounter.${encounterId}`
      const now = new Date().toISOString()
      const row = { _id: `local-${encounterId}`, id: `local-${encounterId}`, encounterId, createdAt: now, updatedAt: now, ...data }
      localStorage.setItem(key, JSON.stringify(row))
      return { prescription: row }
    } catch { throw lastErr || new Error('Failed to upsert prescription by encounter') }
  },
  ...
  listReferrals: (params?: { type?: 'lab' | 'pharmacy' | 'diagnostic'; status?: 'pending' | 'completed' | 'cancelled'; doctorId?: string; prescriptionId?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.type) qs.set('type', params.type)
    if (params?.status) qs.set('status', params.status)
    if (params?.doctorId) qs.set('doctorId', params.doctorId)
    if (params?.prescriptionId) qs.set('prescriptionId', params.prescriptionId)
    if (params?.from) qs.set('from', params.from)
    if (params?.to) qs.set('to', params.to)
    if (params?.page != null) qs.set('page', String(params.page))
    if (params?.limit != null) qs.set('limit', String(params.limit))
  ...
  // Doctor Templates
  listDoctorTemplates: async (doctorId: string, params?: { q?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.q) qs.set('q', params.q)
    if (params?.page != null) qs.set('page', String(params.page))
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    const id = encodeURIComponent(doctorId)
    const attempts: Array<{ path: string; init?: RequestInit; transform?: (r: any) => any }> = [
      { path: `/hospital/doctors/${id}/templates${s ? `?${s}` : ''}`, transform: (r: any) => r },
      { path: `/hospital/doctor-templates${s ? `?${s}&doctorId=${id}` : `?doctorId=${id}`}`, transform: (r: any) => r },
    ]
    let lastErr: any
    for (const a of attempts) {
      try {
        const resp = await api(a.path, a.init)
        return a.transform ? a.transform(resp) : resp
      } catch (e) { lastErr = e }
    }
    // Fallback to localStorage if server route is missing
    try {
      const key = `doctor.templates.${doctorId}`
      const raw = localStorage.getItem(key)
      const arr = raw ? JSON.parse(raw) : []
      const q = String(params?.q || '').trim().toLowerCase()
      const filtered = q ? arr.filter((t: any) => String(t?.name || '').toLowerCase().includes(q)) : arr
      return { templates: filtered }
    } catch { throw lastErr || new Error('Failed to list templates') }
  },
  createDoctorTemplate: async (doctorId: string, data: { name: string; data: any }) => {
    const id = encodeURIComponent(doctorId)
    const attempts: Array<{ path: string; init: RequestInit }> = [
      { path: `/hospital/doctors/${id}/templates`, init: { method: 'POST', body: JSON.stringify(data) } },
      { path: `/hospital/doctor-templates`, init: { method: 'POST', body: JSON.stringify({ doctorId, ...data }) } },
    ]
    let lastErr: any
    for (const a of attempts) {
      try { return await api(a.path, a.init) } catch (e) { lastErr = e }
    ...
    try {
      ...
      if (arr.some(t => String(t?.name || '').trim().toLowerCase() === String(data.name || '').trim().toLowerCase())) {
        const err: any = new Error('Template already exists (409)')
        err.status = 409
        throw err
      }
      const row = { _id: crypto.randomUUID(), doctorId, name: data.name, data: data.data, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
      arr.unshift(row)
      localStorage.setItem(key, JSON.stringify(arr))
      return row
    } catch { throw lastErr || new Error('Failed to create template') }
  },
  updateDoctorTemplate: async (doctorId: string, templateId: string, patch: { name?: string; data?: any }) => {
    const did = encodeURIComponent(doctorId)
    const tid = encodeURIComponent(templateId)
    const attempts: Array<{ path: string; init: RequestInit }> = [
      { path: `/hospital/doctors/${did}/templates/${tid}`, init: { method: 'PUT', body: JSON.stringify(patch) } },
      { path: `/hospital/doctor-templates/${tid}`, init: { method: 'PUT', body: JSON.stringify({ doctorId, ...patch }) } },
    ]
    let lastErr: any
    for (const a of attempts) {
      try { return await api(a.path, a.init) } catch (e) { lastErr = e }
    ...
    try {
      ...
      const raw = localStorage.getItem(key)
      const arr: any[] = raw ? JSON.parse(raw) : []
      const idx = arr.findIndex(t => String(t?._id || t?.id || '') === String(templateId))
      if (idx < 0) throw new Error('Template not found')
      const nextName = patch?.name != null ? String(patch.name || '').trim() : String(arr[idx]?.name || '')
      if (nextName && arr.some(t => String(t?._id || t?.id || '') !== String(templateId) && String(t?.name || '').trim().toLowerCase() === nextName.toLowerCase())) {
        const err: any = new Error('Template already exists (409)')
        err.status = 409
        throw err
      }
      ...
      localStorage.setItem(key, JSON.stringify(arr))
      return arr[idx]
    } catch { throw lastErr || new Error('Failed to update template') }
  },
  deleteDoctorTemplate: async (doctorId: string, templateId: string) => {
    const did = encodeURIComponent(doctorId)
    const tid = encodeURIComponent(templateId)
    const attempts: Array<{ path: string; init: RequestInit }> = [
      { path: `/hospital/doctors/${did}/templates/${tid}`, init: { method: 'DELETE' } },
      { path: `/hospital/doctor-templates/${tid}`, init: { method: 'DELETE' } },
    ]
    let lastErr: any
    for (const a of attempts) {
      try { return await api(a.path, a.init) } catch (e) { lastErr = e }
    ...
    try {
      ...
      const raw = localStorage.getItem(key)
      const arr: any[] = raw ? JSON.parse(raw) : []
      const next = arr.filter(t => String(t?._id || t?.id || '') !== String(templateId))
      localStorage.setItem(key, JSON.stringify(next))
      return { ok: true }
    } catch { throw lastErr || new Error('Failed to delete template') }
  },
