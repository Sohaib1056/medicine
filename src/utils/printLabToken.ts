...
export async function printLabTokenSlip(order: LabSlipOrderInput){
  ...
  const urgent = Number((order as any).urgentCharges || 0)
  const received = Number((order as any).receivedAmount || 0)
  const receivable = Number((order as any).receivableAmount || 0)
  const netAmount = Number((order as any).net || 0)
  const remainingPayable = Math.max(0, netAmount - Math.max(0, received))
  ...
  <style>
    ...
    .container{padding:16px 20px;font-family:'Poppins',ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial;color:#0f172a;max-height:70vh;overflow-y:auto}
    ...
    @media print{
      ...
      .container{ max-height:none !important; overflow:visible !important }
  ...
  <div class="slip-card print-area" id="lab-slip-printable">
    ...
    <div class="container">
      ...
      ${urgent > 0 ? `<div class="frow"><div>Urgent Charges:</div><div>${urgent.toFixed(2)}</div></div>` : ''}
      <div class="frow"><div>Net Amount:</div><div>${netAmount.toFixed(2)}</div></div>
      ${received > 0 ? `<div class="frow"><div>Received:</div><div>${received.toFixed(2)}</div></div>` : ''}
      ${receivable > 0 ? `<div class="frow"><div>Receivable:</div><div>${receivable.toFixed(2)}</div></div>` : ''}
      <div class="frow total"><div>Payable Amount:</div><div>${remainingPayable.toFixed(2)}</div></div>
