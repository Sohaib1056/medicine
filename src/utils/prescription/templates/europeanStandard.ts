...
export async function buildEuropeanStandard(data: PrescriptionPdfData){
  ...
  // removed unused hasUrdu/setUrdu helpers
  ...
  const tableData = items.map((item, idx) => {
    const dose = item.dose || (item as any).qty || ''
    const duration = item.duration || ''
    const instr = item.instruction || ''
    const tpd = timesPerDay(String(item.frequency || ''), instr)
    const days = parseDays(duration)
    const doseNum = parseDose(dose as any)
  ...
  if (tableData.length > 0) {
    autoTable(pdf, {
      ...
      columnStyles: {
        ...
        2: { cellWidth: 24 },
        3: { cellWidth: 40 },
        4: { cellWidth: 12, halign: 'right' as any }
      },
