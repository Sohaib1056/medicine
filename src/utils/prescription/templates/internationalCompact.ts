...
// removed unused autoTable import
...
export async function buildInternationalCompact(data: PrescriptionPdfData){
  ...
  const timesPerDay = (freq?: string, instr?: string) => {
    const s = `${freq || ''} ${instr || ''}`.toLowerCase()
    if (/(^|\s)(qid|qds)(\s|$)/.test(s)) return 4
    if (/(^|\s)(tds|tid)(\s|$)/.test(s)) return 3
    if (/(^|\s)(bd|bid)(\s|$)/.test(s)) return 2
    if (/(^|\s)(od|hs|daily|once\s*a\s*day)(\s|$)/.test(s)) return 1
    if (s.includes('+')) return s.split('+').filter(Boolean).length
    if (s.includes('/')) return s.split('/').filter(Boolean).length
    const eng = [/\bmorning\b/, /\bnoon\b|\bafternoon\b/, /\bevening\b/, /\bnight\b|\bhs\b/].reduce((c, r) => (r.test(s) ? c + 1 : c), 0)
    const urdu = [/صبح/, /دوپہر/, /شام/, /رات/].reduce((c, r) => (r.test(s) ? c + 1 : c), 0)
    return Math.max(eng, urdu) || 1
  }
  const parseDose = (raw?: string) => {
    const r = String(raw || '').replace(/tablets?|capsules?|drops?|sachets?|syrups?|sy?p|injections?|inj|creams?/gi, '').replace(/\(\s*s\s*\)/gi, '').trim()
    if (/1\/2/.test(String(raw || '')) || /(^|[^0-9])0?\.5([^0-9]|$)/.test(String(raw || ''))) return 0.5
  ...
  const parseDays = (raw?: string) => {
    ...
    if (l.includes('week')) v *= 7
    if (l.includes('month')) v *= 30
    return Math.max(0, v)
  }
  ...
  items.slice(0, 8).forEach((item, idx) => {
    ...
    const doseNum = parseDose(item.dose as any || (item as any).qty)
    const days = parseDays(duration)
    const tpd = timesPerDay(String(item.frequency || ''), instruction)
    const qty = (days > 0) ? Math.ceil(doseNum * tpd * days) : ''
    ...
    // Dose, duration and Qty (small text below)
    setPoppins('normal')
    pdf.setFontSize(6)
    pdf.setTextColor(100, 105, 115)
    const meta = [dose, duration, qty ? `Qty: ${qty}` : ''].filter(Boolean).join(' • ')
